...
sudo echo "GenerateName=no" >> /etc/iscsi/initiatorname.iscsi
sudo echo "InitiatorName=iqn.1994-05.com.redhat:client1" >> /etc/iscsi/initiatorname.iscsi

sudo echo "node.session.auth.authmethod = CHAP" >> /etc/iscsi/iscsid.conf
sudo echo "node.session.auth.username = ${ISCSI_USER}" >> /etc/iscsi/iscsid.conf
sudo echo "node.session.auth.password = ${ISCSI_PASSWORD}" >> /etc/iscsi/iscsid.conf
