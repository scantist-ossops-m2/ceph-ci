...
if ! command -v cephadm && ls /home/ubuntu/cephtest/cephadm; then
    cp /home/ubuntu/cephtest/cephadm /usr/sbin/
fi

# make sure we haven't already created luns
! ls /dev/disk/by-path | grep iscsi

sudo dnf install jq -y

ISCSI_CONT_ID=$(sudo podman ps -qa --filter='name=iscsi' | head -n 1)
ISCSI_DAEMON_NAME=$(cephadm ls --no-detail | jq -r '.[] | select(.name | startswith("iscsi")) | .name')
ISCSI_DAEMON_ID=$(cut -d '.' -f2- <<< "$ISCSI_DAEMON_NAME")
HOSTNAME=$(cephadm shell -- ceph orch ps --daemon-id "$ISCSI_DAEMON_ID" -f json | jq -r '.[] | .hostname')
NODE_IP=$(cephadm shell -- ceph orch host ls --format json | jq --arg HOSTNAME "$HOSTNAME" -r '.[] | select(.hostname == $HOSTNAME) | .addr')
...
FQDN=$(python3 -c 'import socket; print(socket.getfqdn())')
ISCSI_POOL=$(cephadm shell -- ceph orch ls iscsi --format json | jq -r '.[] | .spec | .pool')
...
container_gwcli() {
    podman exec -it ${ISCSI_CONT_ID} gwcli "$@"
}
...
echo "GenerateName=no" >> /etc/iscsi/initiatorname.iscsi
echo "InitiatorName=iqn.1994-05.com.redhat:client1" >> /etc/iscsi/initiatorname.iscsi

echo "node.session.auth.authmethod = CHAP" >> /etc/iscsi/iscsid.conf
echo "node.session.auth.username = ${ISCSI_USER}" >> /etc/iscsi/iscsid.conf
echo "node.session.auth.password = ${ISCSI_PASSWORD}" >> /etc/iscsi/iscsid.conf
...
iscsiadm -m discovery -t st -p ${NODE_IP}
