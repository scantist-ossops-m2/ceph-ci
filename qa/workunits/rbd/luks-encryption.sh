...
function test_encryption_format() {
  ...
  drop_caches
  dd if=/dev/mapper/cryptsetupdev of=/tmp/cmpdata iflag=direct bs=4M
  sudo cmp -n 16MB /tmp/cmpdata /tmp/testdata1

  # write via cryptsetup && compare
  sudo dd if=/tmp/testdata2 of=/dev/mapper/cryptsetupdev conv=fdatasync
  drop_caches
  dd if=$LIBRBD_DEV of=/tmp/cmpdata iflag=direct bs=4M
  sudo cmp -n 16MB /tmp/cmpdata /tmp/testdata2
}
