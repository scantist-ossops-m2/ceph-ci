...
DHM_POOL=$(ceph osd pool autoscale-status | grep -oe 'device_health_metrics' || true)
...
PROFILE1=$(ceph osd pool autoscale-status | grep 'device_health_metrics' | grep -oe 'scale-up\|scale-down' || true)
...
PROFILE2=$(ceph osd pool autoscale-status | grep 'test_pool' | grep -oe 'scale-up\|scale-down' || true)
...
then
  ...
  echo "Error: test_pool PROFILE is scale-up"
  exit 1
fi
