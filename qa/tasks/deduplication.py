...
def task(ctx, config):
    ...
    def run_remote(args, need_wait, client_num):
        ...
        while True:
            ...
            log.info('exitstatus {r} need wait {w}'.format(r=proc.exitstatus, w=need_wait))
            if proc.exitstatus == 0 or need_wait == False: 
                log.info('proc exist ', proc.exitstatus, proc.stdout.getvalue())
    ...
    def validate():
        ...
        max_validation_cnt = 20
        retry_cnt = 0

        # check whether sample-dedup has been started
        chunk_obj_list = get_chunk_objs(chunk_pool)
        while (chunk_obj_list == None or len(chunk_obj_list)) and retry_cnt < max_validation_cnt:
        ...
        while retry_cnt < max_validation_cnt:
            for chunk_obj in chunk_obj_list:
                ...
                for ref in ref_list:
                    ret = run_remote(('rados -p ' + base_pool + ' stat ' + ref['oid']).split(),
                                    True, 1)
