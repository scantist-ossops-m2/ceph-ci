...
def setup_ca_signed_keys(ctx, config):
    ...
    bootstrap_remote.run(args=[
        'ssh-keygen -t rsa -f /root/ca-key -N ""'
    ])

    ca_key_pub_contents = bootstrap_remote.read_file('/root/ca-key.pub')
    ...
    bootstrap_remote.run(args=[
        'ssh-keygen -t rsa -f /root/cephadm-ssh-key -N ""',
        run.raw('&&'),
        'ssh-keygen -s /root/ca-key -I user_root -n root -V +52w /root/cephadm-ssh-key'
    ])
    ...
    ctx.ca_signed_key_info['ca-signed-cert'] = '/root/ca-key'
...
def ceph_bootstrap(ctx, config):
    ...
    try:
        ...
        if not config.get("use-ca-signed-key", False):
            cmd.insert('--output-pub-ssh-key', '{}/{}.pub'.format(testdir, cluster_name))
        else:
...
def task(ctx, config):
    ...
    with contextutil.nested(
            ...
            lambda: _bypass() if not (config.get('use-ca-signed-keys', False))\
