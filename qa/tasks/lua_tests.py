...
def configure(ctx, config):
    ...
    for client, properties in config['clients'].items():
        ...
        remote.run(
            args=[
                'cd',
                '{tdir}/ceph/src/test/rgw/lua'.format(tdir=testdir),
                run.Raw('&&'),
                './bootstrap',
                ],
            )
...
def run_tests(ctx, config):
    ...
    for client, client_config in config.items():
        ...
        args = [
            'LUATESTS_CONF={tdir}/ceph/src/test/rgw/lua/lua-tests.{client}.conf'.format(tdir=testdir, client=client),
            '{tdir}/ceph/src/test/rgw/lua/virtualenv/bin/python'.format(tdir=testdir),
            '-m', 'nose',
            '-s',
            '{tdir}/ceph/src/test/rgw/lua/test_lua.py'.format(tdir=testdir),
            '-v',
            '-a', ','.join(attr),
            ]
