    ...
    def test_health_status_after_backtrace_repair(self):
        ...
        damage = json.loads(
            self.fs.mon_manager.raw_cluster_cmd(
                'tell', 'mds.{0}'.format(self.fs.get_active_names()[0]),
                "damage", "ls", '--format=json-pretty'))
        print(f"checkkbacktrace1={damage}")
        self.assertEqual(len(damage), 1)
        ...
        damage = json.loads(
            self.fs.mon_manager.raw_cluster_cmd(
                'tell', 'mds.{0}'.format(self.fs.get_active_names()[0]),
                "damage", "ls", '--format=json-pretty'))
        print(f"checkkbacktrace2={damage}")
        self.assertEqual(len(damage), 0)
        self.assertNotIn("MDS_DAMAGE", self.mds_cluster.mon_manager.get_mon_health()['checks'])
