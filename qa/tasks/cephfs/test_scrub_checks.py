...
""")
    ...
    def test_scrub_when_mds_is_inactive(self):
        ...
        replay = self.fs.get_replay(status=status)['name']

        # start the scrub and verify
        with self.assertRaises(CommandFailedError) as ce:
            #self.fs.run_scrub(["start", abs_test_path, "recursive"])
            self.run_ceph_cmd('tell', f'mds.{replay}', 'scrub', 
                              'start', 'abs_test_path', 'recursive')
        self.assertEqual(ce.exception.exitstatus, errno.EINVAL)

        # pause and verify
        with self.assertRaises(CommandFailedError) as ce:
            self.run_ceph_cmd('tell', f'mds.{replay}', 'scrub', 'pause')
        self.assertEqual(ce.exception.exitstatus, errno.EINVAL)
        
        # abort and verify
        with self.assertRaises(CommandFailedError) as ce:
            self.run_ceph_cmd('tell', f'mds.{replay}', 'scrub', 'abort')
        self.assertEqual(ce.exception.exitstatus, errno.EINVAL)
        
        # resume and verify
        with self.assertRaises(CommandFailedError) as ce:
            self.run_ceph_cmd('tell', f'mds.{replay}', 'scrub', 'resume')
        self.assertEqual(ce.exception.exitstatus, errno.EINVAL)
