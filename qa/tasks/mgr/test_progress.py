...
class TestProgress(MgrTestCase):
    ...
    def _get_osd_in_out_events(
        ...
        if marked == 'both':
            if events_list is not None:
                events_list = [marked_in_events] + [marked_out_events]
            return [marked_in_events] + [marked_out_events]
        elif marked == 'in':
            if events_list is not None:
                events_list = marked_in_events
            return marked_in_events
        else:
            if events_list is not None:
                events_list = marked_out_events
    ...
    def _simulate_failure(self, osd_id=None):
        ...
        with self.recovery_backfill_disabled():
            ...
            try:
                ...
                self.wait_until_equal(lambda: len(self._get_osd_in_out_events('out',
                                      'in progress', events)), 1,
    ...
    def _simulate_back_in(self, osd_id, initial_event):
        ...
        with self.recovery_backfill_disabled():
            try:
                ...
                self.wait_until_equal(lambda: len(self._get_osd_in_out_events('in', 'in progress', events)), 1,
    ...
    def test_osd_healthy_recovery(self):
        ...
        self.assertEqual(len(self._get_osd_in_out_events()), 0)
