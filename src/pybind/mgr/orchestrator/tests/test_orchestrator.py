...
@mock.patch("orchestrator.OrchestratorCli.get_inventory", return_value=ilist)
def test_orch_device_ls(_describe_service):
    ...
    m = OrchestratorCli('orchestrator', 0, 0)
    r = m._handle_command(None, cmd)
    out = 'HOST          PATH      TYPE     DEVICE ID   SIZE  AVAILABLE  REJECT REASONS  \n'\
        'ceph-node-1   /dev/vdb  unknown  None          0   Yes                        \n'\
        'ceph-node-2   /dev/vdb  unknown  None          0   Yes                        \n'\
        'ceph-node-10  /dev/vdb  unknown  None          0   Yes                        '
    assert r == HandleCommandResult(retval=0, stdout=out, stderr='')
