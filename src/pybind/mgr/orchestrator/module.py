...
Usage:
  ...
  ceph orch daemon add osd host:block=device1,device2,db=device3,osds_per_device=2,...
"""
        if not svc_arg:
        ...
        try:
            ...
            values = devices.split(',')
            while values:
                v = values[0].split(',', 1)[0]
                if '=' in v:
                    drv_grp_spec_arg, device = v.split('=')
                    if drv_grp_spec_arg in ['data_devices',
                                            'db_devices',
                                            'wal_devices',
                                            'journal_devices']:
                        drive_group_spec[drv_grp_spec_arg] = []
                        drive_group_spec[drv_grp_spec_arg].append(device)
                    else:
                        drive_group_spec[drv_grp_spec_arg] = device
