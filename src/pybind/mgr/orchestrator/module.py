...
class HostDetails:
    ...
    def _load(self, object_dump: Dict[str, Any]):
    ...
    def _build(self):
        ...
        if self._facts:
            ...
            _total_bytes = self._facts.get('memory_total_kb') * 1024
...
class OrchestratorCli(OrchestratorClientMixin, MgrModule,
    ...
    def _get_hosts(self,
        ...
        if show_detail:
            ...
            _hosts = []
            for h in hosts:
                completion = self.get_facts(h.hostname)
                host_facts = raise_if_exception(completion)
                _hosts.append(HostDetails(h, host_facts[0]))
            hosts = _hosts
