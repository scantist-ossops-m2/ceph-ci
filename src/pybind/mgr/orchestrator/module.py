...
class OrchestratorCli(OrchestratorClientMixin, MgrModule,
    ...
    def _get_hosts(self, format: Format = Format.plain, matches_name: str = '', label: str = '', host_status: str = '') -> HandleCommandResult:
        """List hosts"""
        completion = self.get_hosts()
        hosts = raise_if_exception(completion)

        if label:
            hosts = [h for h in hosts if label in h.labels]

        if matches_name:
            hosts = [h for h in hosts if re.search(matches_name, h.hostname)]
        ...
        if matches_name:
            output += f' whose hostname matched {matches_name}'
        if host_status:
