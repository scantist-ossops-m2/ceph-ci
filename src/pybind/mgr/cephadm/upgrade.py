...
class CephadmUpgrade:
    ...
    def upgrade_ls(self, image: Optional[str], tags: bool) -> Dict:
        ...
        if not tags:
            for t in ls:
                ...
                v_major = int(v[0])
                v_minor = int(v[1])
                if v_major > current_major or (v_major == current_major and v_minor >= current_minor):
    ...
    def upgrade_start(self, image: str, version: str, daemon_types: Optional[List[str]] = None, hosts: Optional[List[str]] = None, services: Optional[List[str]] = None) -> str:
