            ...
            with with_service(cephadm_module, NvmeofServiceSpec(service_id=pool,
                ...
                _run_cephadm.assert_called_with(
                    ...
                    [
                        ...
                        '--meta-json', f'{"{"}"service_name": "nvmeof.{pool}", "ports": [{default_port}], "ip": null, "deployed_by": [], "rank": null, "rank_generation": null, "extra_container_args": null, "extra_entrypoint_args": null{"}"}',
                        '--config-json', '-', '--tcp-ports', f'{default_port}'
                    ],
                    stdin=json.dumps({"config": "", "keyring": f"[client.nvmeof.{nvmeof_daemon_id}]\nkey = None\n", "files": {"ceph-nvmeof.conf": nvmeof_gateway_conf}}),
                    image='')
