...
class GrafanaService(CephadmService):
    ...
    def generate_config(self, daemon_spec: CephadmDaemonDeploySpec) -> Tuple[Dict[str, Any], List[str]]:
        ...
        if os.path.exists(self.mgr.grafana_dashboards_path):
            files = os.listdir(self.mgr.grafana_dashboards_path)
            for file_name in files:
                with open(os.path.join(self.mgr.grafana_dashboards_path, file_name), 'r', encoding='utf-8') as f:
                    dashboard = f.read()
                    logger.error("destination file Namesdasdasdasdadasdasdasd %s", file_name)
                    config_file['files'][f'/etc/grafana/provisioning/dashboards/{file_name}'] = dashboard
            # with open(self.mgr.grafana_dashboards_path, 'r', encoding='utf-8') as f:
            #     dashboards = f.read()
            # config_file['files']['/etc/grafana/dashboards/ceph-dashboard'] = dashboards
