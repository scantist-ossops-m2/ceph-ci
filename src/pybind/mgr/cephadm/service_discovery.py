...
<p><a href='prometheus/sd-config?service=nfs'>NFS exporter http sd-config</a></p>
<p><a href='prometheus/rules'>Prometheus rules</a></p>
    ...
    def get_sd_config(self, service: str) -> List[Dict[str, Collection[str]]]:
        ...
        if service == 'mgr-prometheus':
            ...
            return self.nfs_exporter_sd_config()
        else:
            return []
    ...
    def nfs_exporter_sd_config(self) -> List[Dict[str, Collection[str]]]:
        """Return <http_sd_config> compatible prometheus config for nfs-exporter service."""
        srv_entries = []
        for dd in self.mgr.cache.get_daemons_by_service('nfs'):
            assert dd.hostname is not None
            addr = dd.ip if dd.ip else self.mgr.inventory.get_addr(dd.hostname)
            port = dd.ports[0] if dd.ports else NFSService.DEFAULT_EXPORTER_PORT
            srv_entries.append({
                'targets': [build_url(host=addr, port=port).lstrip('/')],
                'labels': {'instance': dd.hostname}
            })
