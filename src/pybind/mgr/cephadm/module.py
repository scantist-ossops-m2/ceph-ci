...
> ssh -F ssh_config -i ~/cephadm_private_key {self.ssh_user}@{addr}'''
    ...
    def remove_host(self, host: str, force: bool = False, offline: bool = False) -> str:
        ...
        if host_offline and not offline:
            raise OrchestratorValidationError("{} is offline, please use --offline and --force to remove this host. This can potentially cause data loss".format(host))

        if not host_offline and offline:
            raise OrchestratorValidationError("{} is online, please remove host without --offline.".format(host))
        ...
        if not force:
            ...
            if not p.filter_matching_hostspecs(self.inventory.all_specs()):
                raise OrchestratorValidationError("You are about remove a host without having any host with the"
                                                  "'_admin' label. Please add the '_admin' label to a host or"
                                                  "add --force to this command")
