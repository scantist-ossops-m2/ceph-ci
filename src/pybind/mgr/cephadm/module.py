...
> ssh -F ssh_config -i ~/cephadm_private_key {self.ssh_user}@{addr}'''
    ...
    def remove_host(self, host: str, force: bool = False, offline: bool = False) -> str:
        ...
        if not force:
            p = PlacementSpec(label=SpecialHostLabels.admin_label)
            admin_hosts = p.filter_matching_hostspecs(self.inventory.all_specs())
            if len(admin_hosts) == 1 and admin_hosts[0] == host:
                raise OrchestratorValidationError(f"Host {host} is the last host with the '{SpecialHostLabels.admin_label}'"
                                                  f" label. Please add the '{SpecialHostLabels.admin_label}' label to a host"
    ...
    def remove_host_label(self, host: str, label: str, force: bool = False) -> str:
        ...
        if not force and label == SpecialHostLabels.admin_label:
            p = PlacementSpec(label=SpecialHostLabels.admin_label)
            admin_hosts = p.filter_matching_hostspecs(self.inventory.all_specs())
            if len(admin_hosts) == 1 and admin_hosts[0] == host:
                raise OrchestratorValidationError(f"Host {host} is the last host with the '{SpecialHostLabels.admin_label}'"
                                                  f" label.\nRemoving the {SpecialHostLabels.admin_label} label from this host could cause the removal"
                                                  " of the last cluster config/keyring managed by cephadm.\n"
                                                  f"It is recommended to add the {SpecialHostLabels.admin_label} label to another host"
    ...
    def enter_host_maintenance(self, hostname: str, force: bool = False, yes_i_really_mean_it: bool = False) -> str:
        ...
        if yes_i_really_mean_it and not force:
            raise OrchestratorError("--force must be passed with --yes_i_really_mean_it")
    ...
    def drain_host(self, hostname: str, force: bool = False, keep_conf_keyring: bool = False) -> str:
        ...
        if not force:
            ...
            if len(admin_hosts) == 1 and admin_hosts[0] == hostname:
                raise OrchestratorValidationError(f"Host {hostname} is the last host with the '{SpecialHostLabels.admin_label}'"
        ...
        if not keep_conf_keyring:
            self.add_host_label(hostname, SpecialHostLabels.drain_conf_keyring_label)
