...
class FSPerfStats(object):
    ...
    def notify_cmd(self, cmdtag):
        ...
        with self.meta_lock:
            ...
            for metadata in json.loads(client_meta[1]):
                ...
                client_ip = metadata["entity"]["addr"]["addr"]
                    ...
                    for client in self.client_metadata['to_purge']:
                        try:
                            ...
                            if client in self.client_metadata['metadata'][fs_name]:
                                self.client_metadata['metadata'][fs_name].remove(client)
                        except:
                            pass
    ...
    def update_client_meta(self):
        ...
        with self.meta_lock:
            ...
            for fs in fsmap['filesystems']:
                ...
                new_updates[tag] = (fs_list[-1], gid, result)
    ...
    def generate_report(self, user_query):
        ...
        with self.meta_lock:
            ...
            for fs_name in fs_list:
                if fs_name in self.client_metadata["metadata"]:
                    client_meta = result_meta.setdefault(fs_name, {})
                    client_meta.update(self.client_metadata["metadata"][fs_name])

            # start populating global perf metrics w/ client metadata
            fs_metrics = result.setdefault("global_metrics", {})
            for fs_name in fs_list:
                if fs_name in self.client_metadata["metadata"]:
                    for client_id, counters in raw_perfs.items():
                        metrics = fs_metrics.setdefault(fs_name, {})
                        if client_id in self.client_metadata["metadata"][fs_name]:
