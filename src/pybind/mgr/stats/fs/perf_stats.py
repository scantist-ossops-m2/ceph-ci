    ...
    def notify_cmd(self, cmdtag):
        ...
        with self.meta_lock:
            ...
            for metadata in json.loads(client_meta[1]):
                ...
                client_ip_version = metadata["entity"]["addr"]["type"]
                client_ip = (metadata["entity"]["addr"]["addr"].split(':'))[0]
                self.set_client_metadata(fs_name, client_id, "IP", ":".join([client_ip_version,client_ip]))
            ...
            for fs_name in fs_list:
                ...
                if fs_name in meta and len(meta[fs_name]):
                    for client_id in raw_perfs.keys():
                        ...
                        if client_id in meta[fs_name]:
                            client_meta.update(meta[fs_name])
