...
struct MockTestEncryptionFormat : EncryptionFormat<MockTestImageCtx> {
  ...
  MOCK_METHOD3(load, void(MockTestImageCtx*, std::string*, Context*));
...
struct ShutDownCryptoRequest<MockTestImageCtx> {
  ...
  std::unique_ptr<EncryptionFormat<MockTestImageCtx>>* format;
  MockTestEncryptionFormat* mock_format;
...
struct TestMockCryptoLuksFlattenRequest : public TestMockFixture {
  ...
  void SetUp() override {
    ...
    mock_encryption_format = new MockEncryptionFormat("old");
  ...
  void expect_shutdown_crypto(int r = 0) {
    if (r == 0) {
      mock_shutdown_crypto_request.mock_format =
              new MockTestEncryptionFormat("");
    }
    EXPECT_CALL(mock_shutdown_crypto_request, send()).WillOnce(
                    Invoke([this, r]() {
                      if (r == 0) {
                        mock_shutdown_crypto_request.mock_format->id =
                                mock_image_ctx->encryption_format->id;
                        mock_shutdown_crypto_request.format->reset(
                                mock_shutdown_crypto_request.mock_format);
  ...
  void expect_get_crypto() {
    EXPECT_CALL(*mock_shutdown_crypto_request.mock_format,
                get_crypto()).WillOnce(
                        Return(&mock_crypto));
  }
...
TEST_F(TestMockCryptoLuksFlattenRequest, LUKS1) {
  ...
  ASSERT_EQ("old", mock_image_ctx->encryption_format.get()->id);
}
