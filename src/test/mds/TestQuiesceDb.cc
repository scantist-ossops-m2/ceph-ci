...
class QuiesceDbTest: public testing::Test {
  protected:
    ...
    epoch_t epoch = 0;
    std::map<mds_rank_t, std::unique_ptr<TestQuiesceDbManager>> managers;
    ...
    std::unordered_map<mds_rank_t, QuiesceMap> latest_acks;
    using AckHook = std::function<bool(mds_rank_t, QuiesceMap&)>;
    ...
    void configure_cluster(std::vector<mds_rank_t> leader_and_replicas = {0,1,2})
    {
      ++epoch;
      ASSERT_GE(leader_and_replicas.size(), 1);
      std::set<mds_rank_t> members(leader_and_replicas.begin(), leader_and_replicas.end());
      auto leader = leader_and_replicas[0];
      for (const auto &[this_rank, mgr] : managers) {
        QuiesceClusterMembership mem = {
          ...
          leader,
          members,
          [epoch = this->epoch, this, leader](auto recipient, auto listing) {
            std::unique_lock l(comms_mutex);
            if (epoch == this->epoch) {
              if (this->managers.contains(recipient)) {
                dout(10) << "listing from the leader (" << leader << ") to " << recipient << " for version " << listing.db_version << " with " << listing.sets.size() << " sets" << dendl;
                this->managers[recipient]->submit_listing(listing);
          ...
          [epoch = this->epoch, this, leader, me = this_rank](auto diff_map) {
            std::unique_lock l(comms_mutex);
            if (epoch == this->epoch) {
              if (this->managers.contains(leader)) {
                ...
                this->managers[leader]->submit_ack_from(me, diff_map);
                comms_cond.notify_all();
                l.unlock();
    ...
    TestQuiesceDbManager::Db& db(mds_rank_t rank) {
      return managers[rank]->internal_db();
    }
...
TEST_F(QuiesceDbTest, ManagerStartup) {
  ...
  ASSERT_NO_FATAL_FAILURE(configure_cluster({ 1 }));
}
...
TEST_F(QuiesceDbTest, QuiesceRequestValidation)
{
  ...
  const auto versions = nullopt_and_default<QuiesceDbVersion>();
...
TEST_F(QuiesceDbTest, RootSanitization)
{
  ...
  EXPECT_TRUE(db(0).sets.at("set1").members.contains("file:/root1"));
  EXPECT_TRUE(db(0).sets.at("set1").members.contains("file:/root2"));
  EXPECT_TRUE(db(0).sets.at("set1").members.contains("file:/root3"));
  EXPECT_TRUE(db(0).sets.at("set1").members.contains("inode:4"));
  EXPECT_TRUE(db(0).sets.at("set1").members.contains("inode:5"));
  EXPECT_TRUE(db(0).sets.at("set1").members.contains("inode:18446744073709551615"));
  EXPECT_TRUE(db(0).sets.at("set1").members.contains("inode:18446744073709551614"));
  EXPECT_TRUE(db(0).sets.at("set1").members.contains("inode:18446744073709551613"));
  EXPECT_TRUE(db(0).sets.at("set1").members.contains("file:/root6/root6"));
...
TEST_F(QuiesceDbTest, SetModification)
{
  ...
  ASSERT_TRUE(db(0).sets.contains("set1"));
  ASSERT_TRUE(db(0).sets.at("set1").members.contains("file:/root1"));
  ...
  ASSERT_TRUE(db(0).sets.at("set1").members.contains("file:/root2"));
  ASSERT_TRUE(db(0).sets.at("set1").members.contains("file:/root3"));
  ASSERT_EQ(db(0).sets.at("set1").members.size(), 3);

  auto latest_v = last_request->response.sets.at("set1").db_version;
  ...
  ASSERT_EQ(latest_v, last_request->response.sets.at("set1").db_version);
  ASSERT_EQ(latest_v, db(0).sets.at("set1").db_version);
  ...
  ASSERT_EQ(db(0).sets.at("set1").members.size(), 3);
  ASSERT_TRUE(db(0).sets.at("set1").members.at("file:/root1").excluded);
  ASSERT_FALSE(db(0).sets.at("set1").members.at("file:/root2").excluded);
  ASSERT_FALSE(db(0).sets.at("set1").members.at("file:/root3").excluded);
  ASSERT_FALSE(db(0).sets.at("set1").members.contains("file:/root4"));
  ...
  ASSERT_EQ(db(0).sets.at("set1").members.size(), 4);
  ASSERT_TRUE(db(0).sets.at("set1").members.at("file:/root1").excluded);
  ASSERT_TRUE(db(0).sets.at("set1").members.at("file:/root2").excluded);
  ASSERT_TRUE(db(0).sets.at("set1").members.at("file:/root3").excluded);
  ASSERT_TRUE(db(0).sets.at("set1").members.contains("file:/root4"));
  ASSERT_FALSE(db(0).sets.at("set1").members.at("file:/root4").excluded);
  ...
  ASSERT_FALSE(db(0).sets.at("set2").members.at("file:/root5").excluded);
  ...
  ASSERT_TRUE(db(0).sets.at("set1").members.at("file:/root4").excluded);
  ASSERT_TRUE(db(0).sets.at("set2").members.at("file:/root5").excluded);
...
TEST_F(QuiesceDbTest, Timeouts) {
  ...
  managers.at(0)->reset_agent_callback(QUIESCING_AGENT_CB);
  ...
  managers.at(0)->reset_agent_callback(SILENT_AGENT_CB);
...
TEST_F(QuiesceDbTest, ReleaseAwait)
{
  ...
  auto releasing_v1 = last_request->response.sets.at("set1").db_version;
  ...
  EXPECT_EQ(releasing_v1, last_request->response.sets.at("set1").db_version );
  ...
  managers.at(0)->reset_agent_callback([&](auto &map){
  ...
  // validate that root1 is still reported to the agents as QUIESCED
  // even though we are releasing set1
  ...
  EXPECT_EQ(QS_QUIESCED, root1_state.load());
...
TEST_F(QuiesceDbTest, LeaderShutdown)
{
  ...
  ASSERT_EQ(outstanding_awaits.size(), managers.at(0)->internal_awaits().size());
  ...
  ASSERT_EQ(managers.at(0)->internal_pending_requests().size(), pending_requests.size());

  // reset the membership of the manager
  // this will block until the db thread exits
  managers.at(0)->update_membership({});
...
{
  ASSERT_NO_FATAL_FAILURE(configure_cluster({ 0, 1, 2 }));
  ...
  ASSERT_EQ(awaits.size(), managers.at(0)->internal_awaits().size());
  ...
  {
    bool ranks_quiesced[2] = {false, false};
    auto did_ack = add_ack_hook([&](auto r, auto const &m) {
      if (r < 2) {
        ranks_quiesced[r] |= m.roots.contains("file:/root1") && (m.roots.at("file:/root1").state == QS_QUIESCED);
      }
      return ranks_quiesced[0] && ranks_quiesced[1];
    });

    // let two of the three peers ack quiescing of the root
    managers.at(0)->reset_agent_callback(QUIESCING_AGENT_CB);
    managers.at(1)->reset_agent_callback(QUIESCING_AGENT_CB);
...

  {
    ...
    // allow acks
    managers.at(2)->reset_agent_callback(QUIESCING_AGENT_CB);
...
TEST_F(QuiesceDbTest, MultiRankRelease)
{
  ...
  auto quiesced_v = db(0).sets.at("set1").db_version;

  // prevent one of the acks
  managers.at(1)->reset_agent_callback(SILENT_AGENT_CB);
  ...
  EXPECT_EQ(QS_RELEASING, last_request->response.sets.at("set1").rstate.state);
  auto releasing_v = last_request->response.sets.at("set1").db_version;
  ...
  EXPECT_EQ(releasing_v, last_request->response.sets.at("set1").db_version);
  ...
  for (auto&& rank : {mds_rank_t(0), mds_rank_t(1), mds_rank_t(2),}) {
    EXPECT_EQ(QS_RELEASING, db(rank).sets.at("set1").rstate.state) << "db of rank " << rank;
