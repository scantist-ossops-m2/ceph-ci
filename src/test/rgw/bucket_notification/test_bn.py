...
def test_ps_s3_data_path_v2_large_migration():
    ...
    for tenant, topic_conf in zip(tenants_list, polling_topics_conf):
        ...
        while result != 0:
            time.sleep(1)
            if tenant == '':
                result = admin(['topic', 'rm', '--topic', topic_conf.topic_name], get_config_cluster())[1]
            else:
                result = admin(['topic', 'rm', '--topic', topic_conf.topic_name, '--tenant', tenant], get_config_cluster())[1]
