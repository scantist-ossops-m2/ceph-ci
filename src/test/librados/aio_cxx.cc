...
public:
  ...
  {
    int ret;
    auto PoolPrefix = fmt::format("{}_", ::testing::UnitTest::GetInstance()->current_test_info()->name());
    m_pool_name = get_temp_pool_name(PoolPrefix);
...
TEST(LibRadosAio, PoolQuotaPP) {
  ...
  ASSERT_EQ("", test_data.init());
  auto PoolPrefix = fmt::format("{}_", ::testing::UnitTest::GetInstance()->current_test_info()->name());
  string p = get_temp_pool_name(PoolPrefix);
...
TEST(LibRadosAio, RoundTripPP3)
{
  Rados cluster;
  auto PoolPrefix = fmt::format("{}_", ::testing::UnitTest::GetInstance()->current_test_info()->name());
  std::string pool_name = get_temp_pool_name(PoolPrefix);
