...
class SyntheticDispatcher final
  ...
  bool got_connect;
  std::map<crimson::net::ConnectionRef, std::list<uint64_t> > conn_sent;
  std::map<uint64_t, bufferlist> sent;
  unsigned index;
  SyntheticWorkload *workload;

  SyntheticDispatcher(bool s, SyntheticWorkload *wl):
      is_server(s), got_new(false),
  ...
  void ms_handle_accept(crimson::net::ConnectionRef conn) {
    ...
    for (auto& it : conn_sent[conn]) {
      sent.erase(it);
    }
    conn_sent.erase(conn);
    got_connect = true;
  }
  ...
  void clear_pending(crimson::net::ConnectionRef con) {
    for (std::list<uint64_t>::iterator it = conn_sent[con].begin();
...
class SyntheticWorkload {
 ...
 public:
   ...
   seastar::future<crimson::net::ConnectionRef> _get_random_connection() {
     return seastar::do_until([this] {
       return dispatcher.get_pending() <= max_in_flight; },
     [] {
   ...
   seastar::future<> generate_connection() {
   ...
   seastar::future<> init_server(const entity_name_t& name,
     ...
     return msgr->bind(entity_addrvec_t{addr}).safe_then(
       ...
       logger().error("{} test_test_validate_transparent_recovery(): "
   ...
   seastar::future<> wait_for_done() {
     ...
     [this, &i] {
       ...
       return seastar::do_for_each(available_servers, [] (auto server) {
         logger().warn("server {} shutdown" , server->get_myaddrs());
         server->stop();
       ...
       return seastar::do_for_each(available_clients, [] (auto client) {
         logger().warn("client {} shutdown" , client->get_myaddrs());
         client->stop();
...
void SyntheticDispatcher::ms_handle_reset(crimson::net::ConnectionRef con, bool is_replace) {
...
// Random Operations: 500 (Generate/Drop Connection, Send Message, Sleep)
seastar::future<> test_stress_1()
{

  logger().info("test_stress_1():");

  SyntheticWorkload test_msg(3, 9, 100,
                             crimson::net::SocketPolicy::stateful_server(0),
                             crimson::net::SocketPolicy::lossless_client(0));

  return seastar::do_with(test_msg, []
	...
	uint64_t server_num = get_nonce();
      ...
      [&test_msg] {
        ...
        return test_msg.generate_connections(50);
      }).then([&test_msg] {
        return test_msg.random_op(100);
      }).then([&test_msg] {
	return seastar::with_timeout(
	  seastar::lowres_clock::now() + 300s,
	  test_msg.wait_for_done());
      }).then([] {
        logger().info("test_stress_1() DONE");
      }).handle_exception([] (auto eptr) {
        logger().error(
          "test_stress_1() failed: got exception {}",
          eptr);
        throw;
      });
    });
...
// Servers: 32             (loseless)
// Connections: 100        (Generated between random clients/server)
// Random Operations: 1000 (Generate/Drop Connection, Send Message, Sleep)
seastar::future<> test_stress_2()
{

  logger().info("test_stress_2():");

  SyntheticWorkload test_msg(16, 32, 100,
                             crimson::net::SocketPolicy::stateful_server(0),
                             crimson::net::SocketPolicy::lossless_client(0));
  ...
  return seastar::do_with(test_msg, []
	...
	return seastar::with_timeout(
	  ...
	  test_msg.wait_for_done());
        ...
        logger().info("test_stress_2() DONE");
      }).handle_exception([] (auto eptr) {
        logger().error(
          "test_stress_2() failed: got exception {}",
          eptr);
        throw;
      });
    });
...
// Test Parameters:
// Clients: 8              (loseless)
// Servers: 32             (statefull)
// Connections: 100        (Generated between random clients/server)
// Random Operations: 1000 (Generate/Drop Connection, Send Message, Sleep)
seastar::future<> test_injection_1()
{

  logger().info("test_injection_1():");

  std::ignore = local_conf().set_val("ms_inject_socket_failures", "30");
  std::ignore = local_conf().set_val("ms_inject_internal_delays", "0.1");

  SyntheticWorkload test_msg(8, 32, 100,
                             crimson::net::SocketPolicy::stateful_server(0),
                             crimson::net::SocketPolicy::lossless_client(0));
  ...
  return seastar::do_with(test_msg, []
	...
	uint64_t server_num = get_nonce();
      ...
      [&test_msg] {
        ...
        return test_msg.random_op(15);
      }).then([&test_msg] {
	return seastar::with_timeout(
	  seastar::lowres_clock::now() + 300s,
	  test_msg.wait_for_done());
      }).then([] {
        logger().info("test_inejction_1() DONE");
        std::ignore = local_conf().set_val("ms_inject_socket_failures", "0");
        std::ignore = local_conf().set_val("ms_inject_internal_delays", "0");
        std::ignore = local_conf().set_val("ms_inject_delay_probability", "0");
        std::ignore = local_conf().set_val("ms_inject_delay_max", "0");
	return seastar::now();
      }).handle_exception([] (auto eptr) {
        logger().error(
          "test_injection_1() failed: got exception {}",
          eptr);
        throw;
      });
    });
...
// Connections: 100        (Generated between random clients/server)
// Random Operations: 1000 (Generate/Drop Connection, Send Message, Sleep)
seastar::future<> test_injection_2()
{

  logger().info("test_injection_2():");

  std::ignore = local_conf().set_val("ms_inject_socket_failures", "30");
  std::ignore = local_conf().set_val("ms_inject_internal_delays", "0.1");
  std::ignore = local_conf().set_val("ms_inject_delay_probability", "1");
  std::ignore = local_conf().set_val("ms_inject_delay_max", "5");
  ...
  return seastar::do_with(test_msg, []
	...
	return seastar::with_timeout(
	  ...
	  test_msg.wait_for_done());
        ...
        logger().info("test_inejction_2() DONE");
        std::ignore = local_conf().set_val("ms_inject_socket_failures", "0");
        std::ignore = local_conf().set_val("ms_inject_internal_delays", "0");
        std::ignore = local_conf().set_val("ms_inject_delay_probability", "0");
        std::ignore = local_conf().set_val("ms_inject_delay_max", "0");
	return seastar::now();
        ...
        logger().error(
          "test_inejction_2() failed: got exception {}",
          eptr);
        throw;
      });
    });
...
seastar::future<int> do_test(seastar::app_template& app)
{
  ...
  .then([conf_file_list] {
    ...
    //auto v2_testpeer_islocal = config["v2-testpeer-islocal"].as<bool>();
      return test_stress_1()
    .then([] {
      return test_stress_2();
    }).then([] {
      return test_injection_1();
    }).then([] {
      return test_injection_2();
    }).then([] {
...
int main(int argc, char** argv)
{
  ...
  return app.run(argc, argv, [&app] {
    // This test normally succeeds within 120 seconds, so kill it after 200
    // seconds in case it is blocked forever due to unaddressed bugs.
    return seastar::with_timeout(seastar::lowres_clock::now() + 200s, do_test(app))
      .handle_exception_type([](seastar::timed_out_error&) {
        logger().error("test_messenger timeout after 200s, abort! "
