...
struct SingleErrorTestCase {
  ...
  virtual test_obj_t inject_error(test_obj_t, bool) const = 0;
...
struct ECHashMismatch : SingleErrorTestCaseT<ECHashMismatch> {
  ...
  test_obj_t inject_error(test_obj_t obj, bool is_primary) const {
...
struct DataDigestMismatch : SingleErrorTestCaseT<DataDigestMismatch> {
  ...
  test_obj_t inject_error(test_obj_t obj, bool is_primary) const {
    ...
    obj.so.digest = so_get_oi(obj.so)->data_digest + 1;
    return obj;
  }
};
...
struct OmapDigestMismatch : SingleErrorTestCaseT<OmapDigestMismatch> {
  ...
  test_obj_t inject_error(test_obj_t obj, bool is_primary) const {
    ...
    obj.so.omap_digest = so_get_oi(obj.so)->omap_digest + 1;
    return obj;
  }
};
...
struct MissingAttribute : SingleErrorTestCaseT<MissingAttribute> {
  ...
  test_obj_t inject_error(test_obj_t obj, bool is_primary) const {
    ceph_assert(!is_primary);
    auto iter = obj.so.attrs.begin();
    for (; iter != obj.so.attrs.end(); ++iter) {
      if (iter->first != OI_ATTR && iter->first != SS_ATTR &&
	  iter->first != ECUtil::get_hinfo_key()) break;
    }
    // Every test object has at least one such attribute
    ceph_assert(iter != obj.so.attrs.end());
    obj.so.attrs.erase(iter);
    return obj;
  }
};
...
TEST_P(TestSingleError, SingleError) {
  const auto &[obj, is_primary, test_case] = GetParam();
  ...
  auto with_error = test_case->inject_error(obj, is_primary);
  ...
  for (const auto &shard : shards) {
	...
	test_case->get_shard_error_sig()));
      ...
      EXPECT_TRUE(siter->second.authoritative);
...
TEST_P(TestSnapSetCloneError, CloneError) {
  ...
  test_clone_list_t should_exist = {
    { 100, 32 }, { 50,  64 }, { 25,  32 }, { 10,  64 }
  };
  test_clone_list_t extra = {
    { 101, 32 }, { 99,  64 }, { 11,  32 }, { 9,  64 }
  };
...
TEST(TestSnapSet, MissingHead) {
  ...
  for (const auto &desc : test_clone_list_t{
      { 100, 32 }, { 50,  64 }, { 25,  32 }, { 10,  64 }}) {
