...
TEST(SimpleBitmap, intersection)
{
  ...
  SimpleBitmap sbmap(g_ceph_context, "bluestore::SBMP::", MAP_SIZE);
...
TEST(SimpleBitmap, bounderies)
{
  ...
  for (unsigned i = 0; i < 64; i++) {
    ...
    memset(ext_arr.get(), 0, sizeof(extent_t)*MAX_EXTENTS_COUNT);
    sbmap.clear_all();
    ASSERT_TRUE(test_extents_bounderies(count, ext_arr.get(), MAX_EXTENTS_COUNT, sbmap, true) == 0);
    
    memset(ext_arr.get(), 0, sizeof(extent_t)*MAX_EXTENTS_COUNT);
    sbmap.set_all();
    ASSERT_TRUE(test_extents_bounderies(count++, ext_arr.get(), MAX_EXTENTS_COUNT, sbmap, false) == 0);
  }
}
