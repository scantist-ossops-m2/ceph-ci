...
int test_extents(uint64_t index, extent_t *ext_arr, uint64_t ext_arr_size, SimpleBitmap& sbmap, bool set)
{
  ...
  for (i = 0; i < ext_arr_size; i++) {
    ...
    if (set) {
      ...
      if (0 ) {
	std::cout << i << "] clear (" << offset << ", " << length << ")"<< std::endl;
      }
  ...
  for(unsigned i = 0; i < arr_size; i++) {
    ...
    if (ext.length == 0) {
      std::cerr << "Null extent was returned: offset=0x" << std::hex << offset << ", i=0x" << i << std::endl;
      for (unsigned j = i - 3; j <= i; j++) {
	const extent_t & ext_ref = ext_arr[j];
	std::cout << j << ") ref_ext = [0x" << std::hex << ext_ref.offset << ", 0x" << ext_ref.length << "]" << std::endl;
      }
      return -1;
    }

    const extent_t & ext_ref = ext_arr[i];
    if (ext.offset != ext_ref.offset || ext.length != ext_ref.length) {
      std::cerr << "mismatch was found!!" << std::endl;
      
      unsigned start = std::max(((int32_t)(i)-3), 0);
      for (unsigned j = start; j <= i; j++) {
	const extent_t & ext_ref = ext_arr[j];
	std::cout << j << ") ref_ext = [" << ext_ref.offset << ", " << ext_ref.length << "]" << std::endl;
      }
      std::cout << i << ") ext     = [" << ext.offset     << ", " << ext.length     << "]" << std::endl;
      return -1;
    }
...
TEST(SimpleBitmap, basic)
{
  ...
  SimpleBitmap sbmap(g_ceph_context, "bluestore::SBMP::", BIT_COUNT);
...
int test_intersections(unsigned test_idx, SimpleBitmap &sbmap, uint8_t map[], uint64_t map_size)
{
  ...
  //std::cout << "set_op_count = 0x" << set_op_count << ", clr_op_count = 0x" << clr_op_count  << std::endl;
...
{
  const uint64_t BIT_COUNT = 1ULL << 30; // 1G bits = 128MB
  SimpleBitmap sbmap(g_ceph_context, "bluestore::SBMP::", BIT_COUNT);

  // use current time as seed for random generator
  std::srand(std::time(nullptr));

  const uint64_t MAP_SIZE = 1ULL << 30;  // 1GB
...
TEST(SimpleBitmap, bounderies)
{
  const uint64_t MAX_EXTENTS_COUNT = 7131;
  std::unique_ptr<extent_t[]> ext_arr = std::make_unique<extent_t[]>(MAX_EXTENTS_COUNT);
  ASSERT_TRUE(ext_arr != nullptr);
  ...
  uint64_t bit_count = 8 << 20; // 8Mb = 1MB
  for (unsigned i = 0; i < 64; i++) {
    SimpleBitmap sbmap(g_ceph_context, "bluestore::SBMP::", bit_count+i);
    memset(ext_arr.get(), 0, sizeof(extent_t)*MAX_EXTENTS_COUNT);
    sbmap.clear_all();
    ASSERT_TRUE(test_extents_bounderies(i, ext_arr.get(), MAX_EXTENTS_COUNT, sbmap, true) == 0);

    memset(ext_arr.get(), 0, sizeof(extent_t)*MAX_EXTENTS_COUNT);
    sbmap.set_all();
    ASSERT_TRUE(test_extents_bounderies(i, ext_arr.get(), MAX_EXTENTS_COUNT, sbmap, false) == 0);
  }
}
