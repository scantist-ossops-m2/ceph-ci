...
public:
  ...
  hobject_t create_hobject(unsigned           idx,
			   snapid_t           snapid,
			   int64_t            pool,
  ...
  void __add_object(const hobject_t & obj, const set<snapid_t> &snaps) {
    hobject_to_snap[obj] = snaps;

    for (set<snapid_t>::iterator snap_itr = snaps.begin();
	 snap_itr != snaps.end();
	 ++snap_itr) {
      map<snapid_t, set<hobject_t> >::iterator j = snap_to_hobject.find(*snap_itr);
  ...
  int __trim_snap(snapid_t snapid, unsigned max_count, vector<hobject_t> & out) {
  ...
  // must be called with lock held to protect access to
  // snap_to_hobject and hobject_to_snap in __trim_snap
  // will keep trimming until reaching max_count or failing a call to __trim_snap()
  void trim_snap_force(snapid_t           snapid,
    ...
    while (tmp.size() < max_count) {
      ...
      __trim_snap(snapid, req_size, tmp);
  ...
  void trim_snap() {
    ...
    do {
      ...
      vector<hobject_t> out;
      ret = __trim_snap(snap->first, max_count, out);
    } while(ret == 0);
  ...
  void test_prefix_itr() {
    ...
    for (unsigned idx = 0; idx < 8; idx++) {
      ...
      __add_object(hobj, snaps);
      stored_objs.push_back(hobj);
    }
    ceph_assert(hobjects.size() == 8);

    // trim 0, 32, 64, 96
    __trim_snap(snapid, 4, trimmed_objs);
  ...
  void test_prefix_itr2() {
    ...
    hobject_t dup_hobj;
    bool      found = false;
    __trim_snap(snapid, 1, trimmed_objs);
  ...
  void test_prefix_itr_rand() {
    ...
    std::lock_guard   l{lock};
    snapid_t          snapid = create_snap();
    //std::cout << std::hex << "::mask=" << mask << ", bits=" << bits << std::dec << std::endl;
    // we initialize 32 PGS
    ceph_assert(bits == 5);
    ...
    __trim_snap(snapid, 75, trimmed_objs);
...
// We are adding objects into Snaps which are in "DELETED" state to make
// sure SnapTrim will find them even when added into positions before the prefix_itr.
