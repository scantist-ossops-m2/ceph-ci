  ...
  for (;;) {
    ...
    while (!compact_queue_clone.empty()) {
      ...
      if (remove) {
        ...
        std::optional<range_t> merged_range;
        if (start == end) {
          t->rmkey(prefix, start);
          dout(20) << __func__ << " rmkey "
            << prefix << " "
            << pretty_binary_string(start)
            << dendl;
        } else {
          dout(20) << __func__ << " rm first range "
            ...
            << pretty_binary_string(end)
            << dendl;
          t->rm_range_keys_unconditionally(prefix, start, end);
          merged_range = range_t{start, end, prefix};
        }
        ++i;
        // trying to merge transactions and/or compaction ranges
        while (i != compact_queue_clone.end()) {
          // search for the first range if not found yet,
          auto& [start1, end1, remove1, prefix1] = *i;
          if (!remove1 || prefix1 != prefix) {
            ++i;
            break;
          } else if (start1 == end1) {
            // attach op to current transaction
            // if all previous ops are there
            t->rmkey(prefix1, start1);
            dout(20) << __func__ << " rmkey "
              << prefix1 << " "
              << pretty_binary_string(start1)
              << dendl;
            i = compact_queue_clone.erase(i);
          } else {
            if (!merged_range) {
              // first range found - lets use it
              merged_range = {start1, end1, prefix1};
            } else {
              merged_range->start = std::min(start1, merged_range->start);
              merged_range->end = std::max(end1, merged_range->end);
              dout(20) << __func__ << " merge "
                << pretty_binary_string(merged_range->start) << "-"
                << pretty_binary_string(merged_range->end)
                << dendl;
            ...
            dout(20) << __func__ << " rm range "
              << prefix1 << " "
              << pretty_binary_string(start1) << "-"
              << pretty_binary_string(end1)
              << dendl;
            t->rm_range_keys_unconditionally(prefix1, start1, end1);
            i = compact_queue_clone.erase(i);
          } // if (!remove1 || prefix1 != prefix) .. else
        } // while (i != compact_queue_clone.end())
        // submit and compact in bulk
        int r = submit_transaction_sync(t);
        ceph_assert(r == 0);
        if (merged_range && !global_compact) {
          compact_range(
...
void RocksDBStore::remove_compact_range_async(
...
void RocksDBStore::remove_key_async(
  ...
  remove_compact_range_async(prefix, key, key); // insert fake compact range task
}
