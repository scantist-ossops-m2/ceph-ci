    ...
    utime_t scrub_clock_now)
{
  ...
  const bool upgrade_to_deep =
      ...
      Scrub::was_randomized_below(aconf.deep_randomize_ratio);
...
void ScrubJob::remove_from_osd_queue()
{
  ...
  deep_target.disable();
  // m_active_target should be clear or cleared here RRR
  dout(15) << fmt::format(
    ...
    utime_t now_is)
{
  ...
  merge_delay_requeue(
      std::move(aborted_target), issue, delay, delay_both_targets_t::no,
      now_is);
}
 ...
 *  But how do we redo the update, if the scrubbing never started?
 */
    ...
    utime_t scrub_clock_now)
{
  ...
  if (!shallow_target.is_queued() || !deep_target.is_queued()) {
