...
// Note: 'restrictions' is unused in this version (step 1 of the refactoring)
std::vector<ScrubTargetId> ScrubQueue::ready_to_scrub(
    [[maybe_unused]] Scrub::OSDRestrictions& restrictions,
    utime_t scrub_tick)
{
  dout(10) << fmt::format(
  ...
  // transformed into a vector of targets (which, in this phase, is the pg id).

  auto to_scrub_copy = collect_ripe_jobs(to_scrub, scrub_tick);
  auto penalized_copy = collect_ripe_jobs(penalized, scrub_tick);
  lck.unlock();
