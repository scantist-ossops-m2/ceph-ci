...
sc::result ReplicaActive::react(const ReplicaReserveReq& ev)
{
  ...
  if (m_reservation_status != reservation_status_t::unreserved) {
    ...
    // it granted or pending - to be cleared. This maneuver will also abort
    ...
    dout(1) << fmt::format(
	    ...
	    << dendl;
    post_event(ev);
    return transit<ReplicaActive>();  // RRR make sure this exists & re-enters
  }
...
sc::result ReplicaActiveOp::react(const ReplicaRelease& ev)
{
  ...
  // or:
  return transit<ReplicaActive>(); // RRR verify we exit ReplicaActive
}
