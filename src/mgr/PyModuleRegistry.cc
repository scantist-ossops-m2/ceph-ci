...
#define WCHAR(s) L ## #s
  ...
  PyConfig_SetArgv(&py_conf, 1, (wchar_t *const *)argv);
  // Add more modules
  ...
  const auto mgr_module_path = g_conf().get_val<std::string>("mgr_module_path");
  std::wstring module_search_paths(std::begin(mgr_module_path), std::end(mgr_module_path));
  status = PyWideStringList_Append(&config.module_search_paths, module_search_paths.data());
  ceph_assert(PyStatus_Exception(status));
  status = Py_InitializeFromConfig(&config);
  ceph_assert(PyStatus_Exception(status));
  dout(10) << "append to sys.path " << std::quoted(mgr_module_path) << dendl;
#else
