...
void MgrClient::_send_update()
{
  ...
  if (session && session->con) {
    ...
    if (service_daemon) {
      ldout(cct, 20) << __func__ << " updating the daemon metadata " << dendl;
      update->service_daemon = service_daemon;
      update->daemon_metadata = daemon_metadata;
    }
    ldout(cct, 20) << __func__ << " last part of function reached " << dendl;
  ...
  const std::map<std::string,std::string>& metadata)
{
  ...
  if (need_metadata_update == true &&
    ...
    ldout(cct,20) << __func__ << " " << need_metadata_update << " " << daemon_metadata.empty() << dendl;
    _send_update();
