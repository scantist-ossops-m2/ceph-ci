...
def command_bootstrap(ctx):
    ...
    if (bool(ctx.ssh_public_key) and bool(ctx.ssh_signed_cert)):
        raise Error('--ssh-public-key and --ssh-signed-cert are currently mututally exclusive. --ssh-public-key is intended '
                    'for standard pubkey encryption where the public key is set as an authorized key on cluster hosts. '
                    '--ssh-signed-cert is intended for CA signed keys use case where cluster hosts are configured to trust '
        ...
        with open(ctx.apply_spec) as f:
            ...
            for h in host_dicts:
                if ctx.ssh_signed_cert:
                    # TODO - should we have key distribution supported for this use case?
                    # it would require the user passing the ca pub key, distributing that key,
                    # altering their sshd config to trust that ca pub key, and then restarting sshd
...
def check_ssh_connectivity(ctx: CephadmContext) -> None:
    ...
    # TODO: SSH authentication in CA signed keys use case
