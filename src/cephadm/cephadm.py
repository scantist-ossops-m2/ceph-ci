...
class CephExporter(object):
    ...
    def __init__(self,
        ...
        self.prio_limit = config_json.get('prio-limit', 10)
...
def check_container_engine(ctx: CephadmContext) -> ContainerEngine:
    ...
    if not isinstance(engine, CONTAINER_PREFERENCE):
        ...
        raise Error(
            'No container engine binary found ({}). Try run `apt/dnf/yum/zypper install <container engine>`'.format(' or '.join(exes)))
...
def create_daemon_dirs(ctx, fsid, daemon_type, daemon_id, uid, gid,
    ...
    if daemon_type in Monitoring.components.keys():
        ...
        if daemon_type == 'prometheus':
            ...
            makedirs(os.path.join(data_dir_root, config_dir,
                     'provisioning/datasources'), uid, gid, 0o755)
            makedirs(os.path.join(data_dir_root, 'data'), uid, gid, 0o755)
...
def get_container_mounts(ctx, fsid, daemon_type, daemon_id,
    ...
    if daemon_type in Monitoring.components and daemon_id:
        ...
        if daemon_type == 'prometheus':
            ...
            mounts[os.path.join(data_dir, 'etc/grafana/provisioning/datasources')
...
def deploy_daemon(ctx, fsid, daemon_type, daemon_id, c, uid, gid,
    ...
    if any([port_in_use(ctx, port) for port in ports]):
        if daemon_type == 'mgr':
            ...
            raise Error("TCP Port(s) '{}' required for {} already in use".format(
                ','.join(map(str, ports)), daemon_type))
    ...
    with open(data_dir + '/unit.run.new', 'w') as f, \
        ...
        if daemon_type in Ceph.daemons:
            ...
            f.write('{install_path} -d -m0770 -o {uid} -g {gid} /var/run/ceph/{fsid}\n'.format(
                install_path=install_path, fsid=fsid, uid=uid, gid=gid))
        ...
        if daemon_type == 'osd':
            ...
            _write_container_cmd_to_bash(ctx, f, tcmu_container,
                                         'iscsi tcmu-runner container', background=True)
...
class Firewalld(object):
    ...
    def enable_service_for(self, daemon_type):
        ...
        if not self.available:
            logger.debug(
                'Not possible to enable service <%s>. firewalld.service is not available' % daemon_type)
            return
        ...
        out, err, ret = call(self.ctx, [self.cmd, '--permanent',
                             '--query-service', svc], verbosity=CallVerbosity.DEBUG)
        if ret:
    ...
    def open_ports(self, fw_ports):
        ...
        if not self.available:
            logger.debug(
                'Not possible to open ports <%s>. firewalld.service is not available' % fw_ports)
            return
        ...
        for port in fw_ports:
            ...
            out, err, ret = call(self.ctx, [self.cmd, '--permanent',
                                 '--query-port', tcp_port], verbosity=CallVerbosity.DEBUG)
            if ret:
    ...
    def close_ports(self, fw_ports):
        ...
        if not self.available:
            logger.debug(
                'Not possible to close ports <%s>. firewalld.service is not available' % fw_ports)
            return
...
class CephadmAgent():
    ...
    def __init__(self, ctx: CephadmContext, fsid: str, daemon_id: Union[int, str] = ''):
        ...
        self.target_port = ''
        self.host = ''
        self.daemon_dir = os.path.join(ctx.data_dir, self.fsid,
        ...
        self.volume_gatherer = AgentGatherer(
            self, lambda: self._ceph_volume(enhanced=False), 'Volume')
...
""".format(
    ...
    def pull_conf_settings(self) -> None:
        ...
        self.volume_gatherer.update_func(
            lambda: self._ceph_volume(enhanced=self.device_enhanced_scan))
        ...
        while not self.stop:
            ...
            try:
                ...
                with urlopen(req, context=ssl_ctx) as response:
                    ...
                    total_request_time = datetime.timedelta(
                        seconds=(time.monotonic() - send_time)).total_seconds()
                    logger.info(
                        f'Received mgr response: "{response_json["result"]}" {total_request_time} seconds after sending request.')
            except Exception as e:
            ...
            run_time_average = sum(self.recent_iteration_run_times, 0.0) / \
                len([t for t in self.recent_iteration_run_times if t])
    ...
    def _daemon_ls_subset(self) -> Dict[str, Dict[str, Any]]:
        ...
        out, err, code = call(
            self.ctx,
            [self.ctx.container_engine.path, 'stats', '--format',
                '{{.ID}},{{.MemUsage}}', '--no-stream'],
            verbosity=CallVerbosity.DEBUG
        )
                ...
                for j in os.listdir(os.path.join(data_dir, i)):
                    if '.' in j and os.path.isdir(os.path.join(data_dir, fsid, j)):
                        ...
                        c = CephContainer.for_daemon(
                            self.ctx, self.ctx.fsid, daemon_type, daemon_id, 'bash')
                        ...
                        if container_id:
                            daemons[j]['memory_usage'] = seen_memusage.get(
        ...
        while not self.stop:
            try:
                ...
                try:
                    ...
                    logger.error(
                        f'{self.gatherer_type} Gatherer encountered exception gathering data: {e}')
                    self.data = None
...
def command_agent(ctx: CephadmContext) -> None:
    ...
    if not os.path.isdir(agent.daemon_dir):
        raise Error(
            f'Agent daemon directory {agent.daemon_dir} does not exist. Perhaps agent was never deployed?')
...
def get_public_net_from_cfg(ctx: CephadmContext) -> Optional[str]:
    ...
    for net in configured_subnets:
        if net in local_subnets:
            ...
            logger.warning(
                f'The public CIDR network {net} (from -c conf file) is not configured locally.')
    if not valid_public_net:
        raise Error(
            f'None of the public CIDR network(s) {configured_subnets} (from -c conf file) is configured locally.')

    # Ensure public_network is compatible with the provided mon-ip (or mon-addrv)
    if ctx.mon_ip:
        if not ip_in_subnets(ctx.mon_ip, public_network):
            raise Error(
                f'The provided --mon-ip {ctx.mon_ip} does not belong to any public_network(s) {public_network}')
    elif ctx.mon_addrv:
        addrv_args = parse_mon_addrv(ctx.mon_addrv)
        for addrv in addrv_args:
            if not ip_in_subnets(addrv.ip, public_network):
                raise Error(
                    f'The provided --mon-addrv {addrv.ip} ip does not belong to any public_network(s) {public_network}')
...
def prepare_dashboard(
    ...
    cmd = ['dashboard', 'ac-user-create', ctx.initial_dashboard_user,
...
def _distribute_ssh_keys(ctx: CephadmContext, host_spec: Dict[str, str], bootstrap_hostname: str) -> int:
    ...
    if bootstrap_hostname != host_spec['hostname']:
        ...
        out, err, code = call(ctx, ['sudo', '-u', ctx.ssh_user, 'ssh-copy-id', '-f',
                              '-i', ssh_key, '-o StrictHostKeyChecking=no', '%s@%s' % (ctx.ssh_user, addr)])
        if code:
...
def command_bootstrap(ctx):
    ...
    for f in [ctx.output_config, ctx.output_keyring,
        ...
        if dirname and not os.path.exists(dirname):
            ...
            try:
                ...
                raise Error(
                    f'Unable to create {dirname} due to permissions failure. Retry with root, or sudo or preallocate the directory.')
    ...
    if '.' in hostname and not ctx.allow_fqdn_hostname:
        raise Error('hostname is a fully qualified domain name (%s); either fix (e.g., "sudo hostname %s" or similar) or pass --allow-fqdn-hostname' %
    ...
    # create some initial keys
    (mon_key, mgr_key, admin_key, bootstrap_keyring,
     admin_keyring) = create_initial_keys(ctx, uid, gid, mgr_id)
    ...
    if ctx.registry_url and ctx.registry_username and ctx.registry_password:
        registry_credentials = {'url': ctx.registry_url,
                                'username': ctx.registry_username, 'password': ctx.registry_password}
...
def registry_login(ctx: CephadmContext, url: Optional[str], username: Optional[str], password: Optional[str]) -> None:
    ...
    try:
        ...
        raise Error('Failed to login to custom registry @ %s as %s with given password' %
...
def command_rm_cluster(ctx):
    ...
    ceph_dirs = [f'/run/ceph/{ctx.fsid}',
                 f'/tmp/var/lib/ceph/{ctx.fsid}', f'/var/run/ceph/{ctx.fsid}']
    for dd in ceph_dirs:
...
def command_prepare_host(ctx: CephadmContext) -> None:
    ...
    if 'expect_hostname' in ctx and ctx.expect_hostname and ctx.expect_hostname != get_hostname():
        logger.warning('Adjusting hostname from %s -> %s...' %
                       (get_hostname(), ctx.expect_hostname))
    ...
    def _process_nics(self):
        ...
        for nic_path in HostFacts._nic_path_list:
            ...
            for iface in os.listdir(nic_path):
                ...
                lower_devs_list = [os.path.basename(link.replace('lower_', '')) for link in glob(
                    os.path.join(nic_path, iface, 'lower_*'))]
                upper_devs_list = [os.path.basename(link.replace('upper_', '')) for link in glob(
                    os.path.join(nic_path, iface, 'upper_*'))]
