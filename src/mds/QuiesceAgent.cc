...
void* QuiesceAgent::agent_thread_main() {
  ...
  while(!stop_agent_thread) {
    ...
    for (auto& [root, info] : working.roots) {
      ...
      std::optional<QuiesceInterface::RequestHandle> issue_cancel;
      if (!should_quiesce && !info->cancel_result) {
        issue_cancel = info->quiesce_request;
      }
      ...
      if (issue_quiesce) {
        ...
        info->unlock();
      } else if (issue_cancel) {
        int rc = quiesce_control.cancel_request(*issue_cancel);
        if (rc != 0) {
          dout(1) << "ERROR (" << rc << ") when trying to cancel quiesce request id: " << *issue_cancel << dendl;
        }
        info->lock();
