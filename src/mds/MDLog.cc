...
void MDLog::_segment_upkeep()
{
  ...
  auto log_events_per_segment = g_conf()->mds_log_events_per_segment;
  auto major_segment_event_ratio = g_conf()->mds_major_segment_event_ratio;
  // start a new segment?
  if (events_since_last_major_segment > log_events_per_segment*major_segment_event_ratio) {
  ...
  } else if (ls->end/period != ls->offset/period || ls->num_events >= log_events_per_segment) {
    dout(10) << __func__ << ": starting new segment, current " << *ls << dendl;
    auto sb = new ESegment();
    _submit_entry(sb, nullptr);
  } else if (g_conf()->mds_debug_subtrees && ls->num_events > 1) {
