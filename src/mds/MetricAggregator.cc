...
void MetricAggregator::shutdown() {
...

  {
    ...
    for (auto [client, pc] : client_perf_counters) {
...
void MetricAggregator::refresh_metrics_for_rank(const entity_inst_t &client,
  ...
  if (ins) {
    ...
    client_perf_counters[client] = plb.create_perf_counters();
    m_cct->get_perfcounters_collection()->add(client_perf_counters[client]);
  }

  // update perf counters
  auto perf_counter_ptr = client_perf_counters[client];
...
void MetricAggregator::remove_metrics_for_rank(const entity_inst_t &client,
  ...
  m_cct->get_perfcounters_collection()->remove(client_perf_counters[client]);
  delete client_perf_counters[client];
