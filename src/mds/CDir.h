...
public:
  ...
  void pop_and_dirty_projected_fnode(const LogSegmentRef& ls, const MutationRef& mut);
  ...
  void mark_dirty(const LogSegmentRef& ls, version_t pv=0);
  void mark_clean();

  bool is_new() { return item_new.is_on_list(); }
  void mark_new(const LogSegmentRef& ls);
