...
void MDCache::_truncate_inode(CInode *in, LogSegment *ls)
{
  ...
  if (pi->fscrypt_last_block.length()) {
    ...
    /* objver is 0 means a file hole or it must contain the last block data */
    if (header.objver) {
  ...
  /*
   * If the objver is 0, that means the new size is located in the
   * file hole and the fscrypt_last_block is empty.
   *
   * No need to update the last block here.
   */
  if (header.objver) {
