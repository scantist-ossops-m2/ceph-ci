...
void  NVMeofGwMap::find_failback_gw(const NvmeGwId &gw_id, const NvmeGroupKey& group_key,   bool &propose)
{
    ...
    for (auto& gw_state_it: gws_states) {
        ...
        if (st.sm_state[gw_state.ana_grp_id] != GW_STATES_PER_AGROUP_E::GW_STANDBY_STATE) {// maybe it is still doing failback
            real_failback = true;
    ...
    if(avail_cnt == 1 || real_failback == false) { // if there is at least other 1 available GW, it should be chosen as failback, but not exactly immediately
        // No other gw currently performs some activity with ana group of coming-up GW - we can just take over on the group
