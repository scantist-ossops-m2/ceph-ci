...
{
  ldout(cct, 10) << __func__ << " to " << mepoch << dendl;
...
{
  ldout(cct, 10) << __func__ << " from " << from << dendl;
...
{
  ldout(cct, 10) << __func__ << " of " << rank << dendl;
  ...
  if (stable_peer_tracker) {
    ldout(cct, 10) << "stable_peer_tracker exists so using that ..." << dendl;
    stable_peer_tracker->get_total_connection_score(rank, &score, &liveness);
  } else {
    ldout(cct, 10) << "stable_peer_tracker does not exists, using peer_tracker ..." << dendl;
						 ...
						 const ConnectionTracker *ct)
{
  ...
  ldout(cct, 10) << "elector->is_current_member(from): "
    << elector->is_current_member(from) << dendl;
  ldout(cct, 10) << "last_election_winner: " << last_election_winner << dendl;
  if ((epoch % 2 == 0) &&
      last_election_winner != elector->get_my_rank() &&
    ...
    ldout(cct, 10) << "Lets see if this out-of-quorum peer is worth it " << dendl;
    int best_scorer = 0;
    double best_score = 0;
    double last_voted_for_score = 0;
    ldout(cct, 10) << "elector->paxos_size(): " << elector->paxos_size() << dendl;
    ...
    ldout(cct, 10) << "best_scorer: " << best_scorer << " best_score: " << best_score
	...
	(best_score - last_voted_for_score < ignore_propose_margin)) {
      ...
      ldout(cct, 10) << "drop this message; it won't change our vote so we defer to leader " << dendl;
      return;
    }
  }
  if (mepoch > epoch) {
    ldout(cct, 10) << "mepoch > epoch" << dendl;
    connectivity_bump_epoch_in_election(mepoch);
  } else if (mepoch < epoch) {
    // got an "old" propose,
    ldout(cct, 10) << "mepoch < epoch (got an old propose)" << dendl;
    if (epoch % 2 == 0 &&    // in a non-election cycle
	!elector->is_current_member(from)) {  // from someone outside the quorum
      // a mon just started up, call a new election so they can rejoin!
      ldout(cct, 5) << " got propose from old epoch, "
  ...
  ldout(cct, 10) << "propose from rank=" << from << ", tracker: "
