...
void ObjectCopyRequest<I>::send_read() {
  ...
  if (read_op.image_interval.empty() || (write_read_snap_ids.first <= m_src_snap_id_start)) {
    handle_read(0);
    return;
  }
...
uint64_t ObjectCopyRequest<I>::compute_starting_end_size(){
  ...
  interval_set<uint64_t> image_data_interval;
  interval_set<uint64_t> image_zero_interval;
