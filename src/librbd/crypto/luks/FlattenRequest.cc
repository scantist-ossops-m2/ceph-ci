...
void FlattenRequest<I>::handle_read_header(int r) {
  ...
  if (r < 0) {
    if (r == -EILSEQ) {
      // image is not luks formatted rbd clone
      finish(0);
      return;
    }
  ...
  // switch magic
  int max_replace_offset = Magic::replace_magic(m_image_ctx->cct, m_bl);
  ...
  m_bl.splice(max_replace_offset, m_bl.length() - max_replace_offset);
  write_header();
}
