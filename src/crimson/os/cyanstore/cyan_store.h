...
class CyanStore final : public FuturizedStore {
  class CyanShardStore : public FuturizedShardStore,
			 public seastar::peering_sharded_service<CyanShardStore> {
  public:
    CyanShardStore(std::string path)
      :path(path){}
  ...
  private:
    seastar::future<std::vector<coll_core_t>> _list_collections();
...
public:
  ...
  CyanShardStore& get_local_shard_store() final{
...
private:
  seastar::sharded<CyanShardStore> shard_stores;
  const std::string path;
  uuid_d osd_fsid;
};
}
