...
seastar::future<store_statfs_t> CyanStore::stat() const
{
  ...
  return shard_stores.map_reduce0(
    [](const CyanStore::CyanShardStore &local_store) {
...
seastar::future<> CyanStore::CyanShardStore::mkfs()
{
  std::string fn =
...
using coll_core_t = FuturizedShardStore::coll_core_t;
seastar::future<std::vector<coll_core_t>>
CyanStore::CyanShardStore::list_collections()
{
  ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
  return seastar::do_with(std::vector<coll_core_t>{}, [this](auto &collections) {
    return container().map([](auto &local_store) {
      return local_store._list_collections();
...
CyanStore::mount_ertr::future<> CyanStore::CyanShardStore::mount()
{
...
seastar::future<> CyanStore::CyanShardStore::umount()
{
...
CyanStore::CyanShardStore::list_objects(
  CollectionRef ch,
...
CyanStore::CyanShardStore::create_new_collection(const coll_t& cid)
{
...
seastar::future<CollectionRef>
CyanStore::CyanShardStore::open_collection(const coll_t& cid)
{
  return seastar::make_ready_future<CollectionRef>(_get_collection(cid));
}

seastar::future<std::vector<coll_core_t>>
CyanStore::CyanShardStore::_list_collections()
{
...
CyanStore::CyanShardStore::read_errorator::future<ceph::bufferlist>
CyanStore::CyanShardStore::read(
  CollectionRef ch,
...
CyanStore::CyanShardStore::read_errorator::future<ceph::bufferlist>
CyanStore::CyanShardStore::readv(
  CollectionRef ch,
  const ghobject_t& oid,
...
CyanStore::CyanShardStore::get_attr_errorator::future<ceph::bufferlist>
CyanStore::CyanShardStore::get_attr(
  CollectionRef ch,
  const ghobject_t& oid,
...
CyanStore::CyanShardStore::get_attrs_ertr::future<CyanStore::CyanShardStore::attrs_t>
CyanStore::CyanShardStore::get_attrs(
  CollectionRef ch,
  const ghobject_t& oid)
{
  auto c = static_cast<Collection*>(ch.get());
...
auto CyanStore::CyanShardStore::omap_get_values(
  CollectionRef ch,
  const ghobject_t& oid,
...
auto CyanStore::CyanShardStore::omap_get_values(
  ...
  -> CyanStore::CyanShardStore::read_errorator::future<std::tuple<bool, omap_values_t>>
{
  auto c = static_cast<Collection*>(ch.get());
  logger().debug("{} {} {}", __func__, c->get_cid(), oid);
  auto o = c->get_object(oid);
...
auto CyanStore::CyanShardStore::omap_get_header(
  CollectionRef ch,
  const ghobject_t& oid)
  -> CyanStore::CyanShardStore::get_attr_errorator::future<ceph::bufferlist>
{
...
seastar::future<> CyanStore::CyanShardStore::do_transaction_no_callbacks(
  CollectionRef ch,
...
int CyanStore::CyanShardStore::_remove(const coll_t& cid, const ghobject_t& oid)
{
...
int CyanStore::CyanShardStore::_touch(const coll_t& cid, const ghobject_t& oid)
{
  logger().debug("{} cid={} oid={}",
                __func__, cid, oid);
  auto c = _get_collection(cid);
...
int CyanStore::CyanShardStore::_write(
  const coll_t& cid,
  const ghobject_t& oid,
  uint64_t offset,
  size_t len,
...
int CyanStore::CyanShardStore::_zero(
  const coll_t& cid,
  const ghobject_t& oid,
  uint64_t offset,
  size_t len)
...
int CyanStore::CyanShardStore::_omap_clear(
  const coll_t& cid,
  const ghobject_t& oid)
{
...
int CyanStore::CyanShardStore::_omap_set_values(
  const coll_t& cid,
  const ghobject_t& oid,
  std::map<std::string, ceph::bufferlist> &&aset)
{
...
int CyanStore::CyanShardStore::_omap_set_header(
  const coll_t& cid,
  const ghobject_t& oid,
...
int CyanStore::CyanShardStore::_omap_rmkeys(
  const coll_t& cid,
  const ghobject_t& oid,
...
int CyanStore::CyanShardStore::_omap_rmkeyrange(
  const coll_t& cid,
  const ghobject_t& oid,
...
int CyanStore::CyanShardStore::_truncate(
  const coll_t& cid,
  const ghobject_t& oid,
  uint64_t size)
{
...
int CyanStore::CyanShardStore::_clone(
  const coll_t& cid,
  const ghobject_t& oid,
...
int CyanStore::CyanShardStore::_setattrs(
  const coll_t& cid,
  const ghobject_t& oid,
  std::map<std::string,bufferlist>&& aset)
{
...
int CyanStore::CyanShardStore::_rm_attr(
  const coll_t& cid,
  const ghobject_t& oid,
...
int CyanStore::CyanShardStore::_rm_attrs(
  const coll_t& cid,
  const ghobject_t& oid)
{
...
int CyanStore::CyanShardStore::_create_collection(const coll_t& cid, int bits)
{
...
CyanStore::CyanShardStore::_get_collection(const coll_t& cid)
{
...
unsigned CyanStore::CyanShardStore::get_max_attr_name_length() const
{
  // arbitrary limitation exactly like in the case of MemStore.
  return 256;
}

CyanStore::CyanShardStore::read_errorator::future<std::map<uint64_t, uint64_t>>
CyanStore::CyanShardStore::fiemap(
  CollectionRef ch,
  const ghobject_t& oid,
  uint64_t off,
  uint64_t len)
