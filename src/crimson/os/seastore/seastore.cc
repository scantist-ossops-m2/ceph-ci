...
SeaStore::SeaShardStore::SeaShardStore(
  std::string root,
...
void SeaStore::SeaShardStore::register_metrics()
{
...
SeaStore::mount_ertr::future<> SeaStore::SeaShardStore::mount()
{
  return device->mount(
  ).safe_then([this] {
    ...
    crimson::ct_error::assert_all{
      "Invalid error in SeaShardStore::mount"
    }
  );
}

seastar::future<> SeaStore::SeaShardStore::umount()
{
  return [this] {
...
seastar::future<>
SeaStore::SeaShardStore::prepare_managers()
{
  ...
  return transaction_manager->mkfs(
    ...
    crimson::ct_error::assert_all{
      "Invalid error in SeaShardStore::prepare_managers"
    }
  );
}
...
{
  ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
  return shard_stores.local().prepare_managers(
...
SeaStore::SeaShardStore::list_collections()
{
  ceph_assert(seastar::this_shard_id() == PRIMARY_CORE);
...
store_statfs_t SeaStore::SeaShardStore::stat() const
{
...
seastar::future<store_statfs_t> SeaStore::stat() const
{
  ...
  return shard_stores.map_reduce0(
    [](const SeaStore::SeaShardStore &local_store) {
...
SeaStore::SeaShardStore::get_coll_bits(CollectionRef ch, Transaction &t) const
{
...
SeaStore::SeaShardStore::list_objects(CollectionRef ch,
...
SeaStore::SeaShardStore::create_new_collection(const coll_t& cid)
{
...
seastar::future<CollectionRef>
SeaStore::SeaShardStore::open_collection(const coll_t& cid)
{
  LOG_PREFIX(SeaStore::open_collection);
  DEBUG("{}", cid);
  return _list_collections().then([cid, this] (auto colls_cores) {
...
seastar::future<std::vector<coll_core_t>>
SeaStore::SeaShardStore::_list_collections()
{
  return seastar::do_with(
...
SeaStore::SeaShardStore::read_errorator::future<ceph::bufferlist>
SeaStore::SeaShardStore::read(
  CollectionRef ch,
...
SeaStore::SeaShardStore::get_attr_errorator::future<ceph::bufferlist>
SeaStore::SeaShardStore::get_attr(
  CollectionRef ch,
  const ghobject_t& oid,
...
SeaStore::SeaShardStore::get_attrs_ertr::future<SeaStore::SeaShardStore::attrs_t>
SeaStore::SeaShardStore::get_attrs(
  CollectionRef ch,
  const ghobject_t& oid)
{
...
seastar::future<struct stat> SeaStore::SeaShardStore::stat(
  CollectionRef c,
  const ghobject_t& oid)
{
  LOG_PREFIX(SeaStore::stat);
...
SeaStore::SeaShardStore::get_attr_errorator::future<ceph::bufferlist>
SeaStore::SeaShardStore::omap_get_header(
  CollectionRef ch,
  const ghobject_t& oid)
{
...
SeaStore::SeaShardStore::read_errorator::future<SeaStore::SeaShardStore::omap_values_t>
SeaStore::SeaShardStore::omap_get_values(
  CollectionRef ch,
  const ghobject_t &oid,
...
SeaStore::SeaShardStore::_omap_get_value_ret
SeaStore::SeaShardStore::_omap_get_value(
  Transaction &t,
  omap_root_t &&root,
...
SeaStore::SeaShardStore::_omap_get_values_ret
SeaStore::SeaShardStore::_omap_get_values(
  Transaction &t,
...
SeaStore::SeaShardStore::omap_list_ret
SeaStore::SeaShardStore::omap_list(
  Onode &onode,
...
SeaStore::SeaShardStore::omap_get_values_ret_t
SeaStore::SeaShardStore::omap_get_values(
  CollectionRef ch,
  const ghobject_t &oid,
  const std::optional<string> &start)
{
  ...
  DEBUG("{} {}", c->get_cid(), oid);
  using ret_bare_t = std::tuple<bool, SeaStore::SeaShardStore::omap_values_t>;
...
SeaStore::SeaShardStore::_fiemap_ret SeaStore::SeaShardStore::_fiemap(
  Transaction &t,
  Onode &onode,
  uint64_t off,
  uint64_t len) const
...
SeaStore::SeaShardStore::read_errorator::future<std::map<uint64_t, uint64_t>>
SeaStore::SeaShardStore::fiemap(
  CollectionRef ch,
  const ghobject_t& oid,
  uint64_t off,
  uint64_t len)
...
void SeaStore::SeaShardStore::on_error(ceph::os::Transaction &t) {
...
seastar::future<> SeaStore::SeaShardStore::do_transaction_no_callbacks(
  CollectionRef _ch,
...
seastar::future<> SeaStore::SeaShardStore::flush(CollectionRef ch)
{
  return seastar::do_with(
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_do_transaction_step(
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_remove(
  internal_context_t &ctx,
  OnodeRef &onode)
{
...
SeaStore::SeaShardStore::omap_set_kvs_ret
SeaStore::SeaShardStore::_omap_set_kvs(
  OnodeRef &onode,
  const omap_root_le_t& omap_root,
  Transaction& t,
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_omap_set_values(
  internal_context_t &ctx,
  OnodeRef &onode,
  std::map<std::string, ceph::bufferlist> &&aset)
{
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_omap_set_header(
  internal_context_t &ctx,
  OnodeRef &onode,
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_omap_clear(
  internal_context_t &ctx,
  OnodeRef &onode)
{
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_omap_rmkeys(
  internal_context_t &ctx,
  OnodeRef &onode,
  omap_keys_t &&keys)
{
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_omap_rmkeyrange(
  internal_context_t &ctx,
  OnodeRef &onode,
  std::string first,
  std::string last)
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_truncate(
  internal_context_t &ctx,
  OnodeRef &onode,
  uint64_t size)
{
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_setattrs(
  internal_context_t &ctx,
  OnodeRef &onode,
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_rmattr(
  internal_context_t &ctx,
  OnodeRef &onode,
  std::string name)
{
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_xattr_rmattr(
  internal_context_t &ctx,
  OnodeRef &onode,
  std::string &&name)
{
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_rmattrs(
  internal_context_t &ctx,
  OnodeRef &onode)
{
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_xattr_clear(
  internal_context_t &ctx,
  OnodeRef &onode)
{
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_create_collection(
  internal_context_t &ctx,
...
SeaStore::SeaShardStore::tm_ret
SeaStore::SeaShardStore::_remove_collection(
  internal_context_t &ctx,
  const coll_t& cid)
{
  return transaction_manager->read_collection_root(
...
SeaStore::SeaShardStore::_get_collection(const coll_t& cid)
{
  return new SeastoreCollection{cid};
}

seastar::future<> SeaStore::SeaShardStore::write_meta(
...
uuid_d SeaStore::SeaShardStore::get_fsid() const
{
  return device->get_meta().seastore_id;
}

void SeaStore::SeaShardStore::init_managers()
{
  transaction_manager.reset();
  collection_manager.reset();
  onode_manager.reset();
