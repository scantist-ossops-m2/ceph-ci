...
template <class Ret, class SuccessFunc, class TestFunc, class FailureFunc>
  ...
  FailureFunc&& failure_func)
{
  ...
  auto test_func_ptr = seastar::make_lw_shared(std::move(test_func));
  ...
  return interruptor::do_for_each(ops, [ox](OSDOp& osd_op) {
    ...
    [success_func=std::move(success_func), rollbacker, this, test_func_ptr, failure_func_ptr]
    ...
    [rollbacker, test_func_ptr, failure_func_ptr]
    ...
    return maybe_rollback_fut.then_interruptible([e, failure_func_ptr, test_func_ptr] {
        return PG::do_osd_ops_iertr::make_ready_future<pg_rep_op_fut_t<Ret>>(
          std::move(seastar::now()),        // submitted_fut
          std::move((*test_func_ptr)(e)),   // error_log_fut
          std::move((*failure_func_ptr)(e)) // all_completed_fut
        );
    });
  }));
}
