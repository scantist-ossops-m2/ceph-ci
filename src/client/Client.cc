			 ...
			 bufferlist *pdirbl)
{
  ...
  request->coarse_stamp = ceph::coarse_mono_clock::now();
...
void Client::tick()
{
  ...
  if (is_mounting() && !mds_requests.empty()) {
    ...
    if (req->coarse_stamp + mount_timeout < now) {
				...
				const std::set <std::string> &changed)
{
  ...
  if (changed.count("client_mount_timeout")) {
    mount_timeout = cct->_conf.get_val<std::chrono::seconds>("client_mount_timeout");
  }
}
