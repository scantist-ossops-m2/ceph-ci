		      ...
		      const UserPerm& perms)
{
  ...
  if (!strncmp(name, "ceph.", 5)) {
    auto get_rank_for_getvxattr_support = [&]() -> mds_rank_t {
      // first, test for the auth mds for the feature
      if (in->auth_cap) {
	if (in->auth_cap->session) {
	  auto& features = in->auth_cap->session->mds_features;

	  if (features.test(CEPHFS_FEATURE_GETVXATTR)) {
	    return in->auth_cap->session->mds_num;
	  }
	}
      }
      // else return any rank with the feature
      for (auto [rank, session] : mds_sessions) {
	if (session) {
	  auto& features = session->mds_features;
	  if (features.test(CEPHFS_FEATURE_GETVXATTR)) {
	    return rank;
	  }
	}
      }
      return (mds_rank_t)-1; // no rank with getvxattr support available
    };

    // if we are looking up a ceph vxattr and there's no
    // getvxattr support available, then its an I/O error
    r = -CEPHFS_EOPNOTSUPP;
    auto rank = get_rank_for_getvxattr_support();
    if (rank != (mds_rank_t)-1) {
      r = _getvxattr(in, perms, name, size, value, rank);
    }
    goto out;
  }
