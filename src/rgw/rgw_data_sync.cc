  ...
  int operate(const DoutPrefixProvider* dpp) override
  {
    reenter(this) {
      do {
        ...
        if (retcode < 0) {
          ldout(sc->cct, 20) << "failed to read bucket shard status: "
		    << cpp_strerror(retcode) << dendl;
	  return set_cr_error(retcode);
        }
        ...
        if (bucket_status.state != BucketSyncState::Incremental) {
          ...
          ldout(sc->cct, 20) << "RGWBucketShardIsDoneCR found sync state = " << bucket_status.state << dendl;
          return set_cr_done();
        }
        ...
        if (bucket_status.incremental_gen != generation) {
          // exit with success to avoid stale shard being
          // retried in error repo if we lost a race
          ldout(sc->cct, 20) << "RGWBucketShardIsDoneCR expected gen: " << generation
	...
	if (retcode < 0 && retcode != -ECANCELED) {
          ldout(sc->cct, 20) << "failed to write bucket sync status: " << cpp_strerror(retcode) << dendl;
          return set_cr_error(retcode);
        } else if (retcode >= 0) {
          return set_cr_done();
        }
