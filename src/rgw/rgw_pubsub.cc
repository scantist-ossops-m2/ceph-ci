    ...
    RGWObjVersionTracker *objv_tracker, optional_yield y) const
{
  if (store->get_zone()->get_zonegroup().supports_feature(
    ...
    void* handle = NULL;
    auto ret = store->meta_list_keys_init(dpp, "topic", std::string(), &handle);
    if (ret < 0) {
      return ret;
    }
    bool truncated;
    ...
    do {
      ...
      ret = store->meta_list_keys_next(dpp, handle, max, topics, &truncated);
      if (ret < 0) {
        ldpp_dout(dpp, 1) << "ERROR: lists_keys_next(): " << cpp_strerror(-ret)
                          << dendl;
        break;
      }
  ...
  const int ret = store->read_topics(tenant, result, objv_tracker, y, dpp);
  if (ret < 0) {
...
int RGWPubSub::write_topics(const DoutPrefixProvider *dpp, const rgw_pubsub_topics& topics,
				     RGWObjVersionTracker *objv_tracker, optional_yield y) const
{
  const int ret = store->write_topics(tenant, topics, objv_tracker, y, dpp);
  if (ret < 0 && ret != -ENOENT) {
...
std::optional<rgw_pubsub_topic_filter> find_unique_topic(
  ...
  if (it != bucket_topics.topics.end()) return it->second;
  return std::nullopt;
}
...
int remove_notification_v2(const DoutPrefixProvider* dpp,
                           rgw::sal::Store* store, rgw::sal::Bucket* bucket,
                           const std::string& notification_id,
                           optional_yield y) {
  rgw_pubsub_bucket_topics bucket_topics;
...
int RGWPubSub::create_topic(const DoutPrefixProvider* dpp,
                            ...
                            optional_yield y) const {
  RGWObjVersionTracker objv_tracker;
  const auto ret = store->write_topic_v2(topic, &objv_tracker, y, dpp);
  if (ret < 0) {
...
int RGWPubSub::remove_topic_v2(const DoutPrefixProvider* dpp,
  ...
  ret = store->remove_topic_v2(name, tenant, &objv_tracker, y, dpp);
  if (ret < 0) {
