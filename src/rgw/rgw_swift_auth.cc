...
template<>
constexpr const char * signature_hash_name<ceph::crypto::HMACSHA1> = sha1_name;
template<>
constexpr const char * signature_hash_name<ceph::crypto::HMACSHA256> = sha256_name;
template<>
constexpr const char * signature_hash_name<ceph::crypto::HMACSHA512> = sha512_name;

template<>
const uint32_t signature_hash_name_size<ceph::crypto::HMACSHA1> = sizeof sha1_name;
template<>
const uint32_t signature_hash_name_size<ceph::crypto::HMACSHA256> = sizeof sha256_name;
template<>
const uint32_t signature_hash_name_size<ceph::crypto::HMACSHA512> = sizeof sha512_name;
...
std::unique_ptr<TempURLEngine::SignatureHelper> TempURLEngine::SignatureHelper::get_sig_helper(std::string_view x) {
  ...
  if (pos == x.npos || pos <= 0) {
    ...
    case CEPH_CRYPTO_HMACSHA1_DIGESTSIZE*2:
      return std::make_unique<TempURLSignature::SignatureHelper_x<ceph::crypto::HMACSHA1rgw::auth::swift::SignatureFlavor::BARE_HEX>>();
    case CEPH_CRYPTO_HMACSHA256_DIGESTSIZE*2:
      return std::make_unique<TempURLSignature::SignatureHelper_x<ceph::crypto::HMACSHA256rgw::auth::swift::SignatureFlavor::BARE_HEX>>();
    case CEPH_CRYPTO_HMACSHA512_DIGESTSIZE*2:
      return std::make_unique<TempURLSignature::SignatureHelper_x<ceph::crypto::HMACSHA512rgw::auth::swift::SignatureFlavor::BARE_HEX>>();
    }
