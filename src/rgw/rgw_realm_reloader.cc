...
RGWRealmReloader::RGWRealmReloader(RGWProcessEnv& env, rgw::SiteConfig& site,
...
void RGWRealmReloader::reload()
{
  ...
  while (!env.store) {
    ...
    int r = site.load(&dp, null_yield, env.cfgstore);
    if (r == 0) {
      ...
      env.store = StoreManager::get_storage(&dp, cct, cfg,
