...
int RGWIndexCompletionThread::process(const DoutPrefixProvider *dpp)
{
  ...
  for (auto c : comps) {
    ...
    int r = store->guard_reshard(this, &bs, c->obj, bucket_info,
			    ...
			    rgw_zone_set* zones_trace)
{
  ...
  *plast_ver = last_ver;
  ...
  if (need_to_remove) {
    string olh_tag(state.olh_tag.c_str(), state.olh_tag.length());
    clear_olh(dpp, obj_ctx, obj, bucket_info, ref, olh_tag, last_ver);
  }
...
int RGWRados::clear_olh(const DoutPrefixProvider *dpp,
  ...
  return clear_olh(dpp, obj_ctx, obj, bucket_info, ref, tag, ver);
}
...
int RGWRados::clear_olh(const DoutPrefixProvider *dpp,
  ...
  int r = get_obj_state(dpp, &obj_ctx, bucket_info, obj, &s, false, null_yield);
  if (r < 0) {
    return r;
  }
