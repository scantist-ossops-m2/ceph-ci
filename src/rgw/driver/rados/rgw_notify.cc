    ...
    while (true) {
      ...
      if (remove_entries) {
        ...
        if (!entries_to_migrate.empty()) {
          ...
          auto ret_of_get_topic = ps.get_topic(
              this, queue_name, topic, optional_yield(io_context, yield),
		      ...
		      const RGWObjTags* req_tags)
{
  ...
  for (const auto& bucket_topic : bucket_topics.topics) {
    ...
    auto ret = ps.get_topic(res.dpp, topic_cfg.name, result, res.yield,
                            /*fetch_bucket_mapping=*/false);
    if (ret < 0) {
      ldpp_dout(res.dpp, 1) << "ERROR: failed to load topic: " << topic_cfg.name
                            << ". error: " << ret << dendl;
      if (ret == -ENOENT) {
      ...
      ldpp_dout(res.dpp, 1)
          << "Warning: Using the stored topic from bucket notification struct."
          << dendl;
