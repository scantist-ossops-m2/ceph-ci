...
private:
  ...
  EntryProcessingResult process_entry(const ConfigProxy& conf, persistency_tracker& entry_persistency_tracker,
                                      const cls_queue_entry& entry, std::string& tenant_name, yield_context yield) {
    ...
    if (event_entry.needs_migration) {
      tenant_name = event_entry.event.userIdentity;
  ...
  void process_queue(const std::string& queue_name, yield_context yield) {
    ...
    while (true) {
      ...
      bool migration_in_process = false;
      for (auto& entry : entries) {
        ...
        spawn::spawn(yield, [this, &notifs_persistency_tracker, &queue_name, entry_idx, total_entries, &end_marker,
                             &remove_entries, &has_error, &waiter, &entry, &tenant_name, &migration_in_process](yield_context yield) {
            const auto token = waiter.make_token();
            auto& persistency_tracker = notifs_persistency_tracker[entry.marker];
            auto result = process_entry(this->get_cct()->_conf, persistency_tracker, entry, tenant_name, yield);
            if (result == EntryProcessingResult::Successful || result == EntryProcessingResult::Expired
                || result == EntryProcessingResult::Migrating) {
              std::string return_status = "ok";
              if (result == EntryProcessingResult::Expired) {
                return_status = "expired";
              } else if (result == EntryProcessingResult::Migrating) {
                entry.needs_migration = true;
                migration_in_process = true;
                return_status = "migrating";
              }
              ldpp_dout(this, 20) << "INFO: processing of entry: " << entry.marker
                << " (" << entry_idx << "/" << total_entries << ") from: " << queue_name << return_status << dendl;
              remove_entries = true;
      ...
      if (remove_entries) {
        ...
        if (migration_in_process) {
          ...
          auto ret_of_get_topic = ps.get_topic(this, queue_name, topic, null_yield);
          const auto& conf = this->get_cct()->_conf;
          ...
          for (auto entry: entries_to_migrate) {
            ...
            event_entry.needs_migration = false;
