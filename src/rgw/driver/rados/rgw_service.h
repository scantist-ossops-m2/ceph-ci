...
struct RGWServices_Def
{
  ...
  int init(CephContext *cct, rgw::sal::RadosStore* store, bool have_cache,
	   bool raw_storage, bool run_sync, librados::Rados* rados, optional_yield y,
...
struct RGWServices
{
  ...
  int do_init(CephContext *cct, rgw::sal::RadosStore* store, bool have_cache, bool raw_storage,
	      bool run_sync, librados::Rados* rados, optional_yield y,
	      const DoutPrefixProvider *dpp);

  int init(CephContext *cct, rgw::sal::RadosStore* store, bool have_cache, bool run_sync,
	   librados::Rados* rados, optional_yield y,
	   const DoutPrefixProvider *dpp) {
    return do_init(cct, store, have_cache, false, run_sync, rados, y, dpp);
  }

  int init_raw(CephContext *cct, rgw::sal::RadosStore* store, bool have_cache, librados::Rados* rados,
	       optional_yield y, const DoutPrefixProvider *dpp) {
    return do_init(cct, store, have_cache, true, false, rados, y, dpp);
  }
  void shutdown() {
    _svc.shutdown();
  }
