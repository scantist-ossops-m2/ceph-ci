			    ...
			    fill_cache_cb_t cb)
{
  int ret = for_each(dpp, [this, &cb, &dpp, &y](const char* name) {
    ...
    (void)pobj->get_owner(dpp, y, &owner);

    rgw_bucket_dir_entry bde{};
    pobj->get_key().get_index_key(&bde.key);
    bde.ver.pool = 1;
    bde.ver.epoch = 1;
    bde.exists = true;
    bde.meta.category = RGWObjCategory::Main;
    bde.meta.size = pobj->get_obj_size();
    bde.meta.mtime = pobj->get_mtime();
    if (owner) {
    ...
    bde.meta.accounted_size = pobj->get_obj_size();
    bde.meta.storage_class = RGW_STORAGE_CLASS_STANDARD;
    bde.meta.appendable = true;
    bufferlist etag_bl;
    if (rgw::sal::get_attr(pobj->get_attrs(), RGW_ATTR_ETAG, etag_bl)) {
