...
/**
 * @defgroup SAL Store abstraction layer
 * @{
 */
 ...
 *
 * The Store Abstraction Layer is an API that separates the top layer of RGW that handles client
 * protocols (such as S3 or Swift) from the bottom layer of RGW that interacts with a
 * backing store.  It allows the creation of multiple backing stores that can co-exist
 * with a single RGW instance, and allows the creation of stacking layers of translators
 * that can modify operation as they pass down the stack.  Examples of translators might
 * be a cache layer, a duplication layer that copies operations to multiple stores, or a
 * policy layer that sends some operations to one store and some to another.
 *
 ...
 * contains data, and it's associated metadat.  The @a Bucket contains Objects, and
 ...
 *
 * Writing is done through a set of filters.  This allows chaining filters to do thinks
 ...
 *
 * RGW can load an process Lua scripts.  This will handle loading/storing scripts.
 */
class LuaScriptManager {
public:
