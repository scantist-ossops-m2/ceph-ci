...
class BucketTrimShardCollectCR : public RGWShardCollectCR {
 ...
 public:
  BucketTrimShardCollectCR(const DoutPrefixProvider *dpp,
    ...
    : RGWShardCollectCR(store->ctx(), MAX_CONCURRENT_SHARDS), dpp(dpp),
...
private:
  ...
  int take_min_generation() {
    ...
    if (log->layout.type == rgw::BucketLogType::InIndex) {
      ...
      ldout(store->ctx(), 0) << "Unable to convert log of unknown type "
			<< log->layout.type
...
int BucketTrimInstanceCR::operate(const DoutPrefixProvider* dpp)
{
  reenter(this) {
    ...
    if (retcode < 0) {
      ldout(store->ctx(), 4) << "failed to find minimum generation" << dendl;
      return set_cr_error(retcode);
    }
    retcode = maybe_remove_generation();
    if (retcode < 0) {
      ldout(store->ctx(), 4) << "error removing old generation from log: "
    ...
    if (clean_info) {
      yield call(new BucketCleanIndexCollectCR(dpp, store, clean_info->first,
					       clean_info->second));
      if (retcode < 0) {
	ldout(store->ctx(), 0) << "failed to remove previous generation: "
			  << cpp_strerror(retcode) << dendl;
	return set_cr_error(retcode);
      }
	...
	if (retcode == -ECANCELED) {
	  ...
	  if (retcode < 0) {
	    ldout(store->ctx(), 0) << "failed to get bucket info: "
			      << cpp_strerror(retcode) << dendl;
	    return set_cr_error(retcode);
	  }
	...
	if (retcode < 0) {
	  ldout(store->ctx(), 0) << "failed to put bucket info: "
			    << cpp_strerror(retcode) << dendl;
	  return set_cr_error(retcode);
	}
	clean_info = std::nullopt;
      ...
      if (retcode < 0) {
	ldout(store->ctx(), 4) << "failed to correlate bucket sync status from peers" << dendl;
	return set_cr_error(retcode);
      }

      // trim shards with a ShardCollectCR
      ldout(store->ctx(), 10) << "trimming bilogs for bucket=" << pbucket_info->bucket
	...
	retcode = 0;
      ...
      if (retcode < 0) {
	ldout(store->ctx(), 4) << "failed to trim bilog shards: "
			  << cpp_strerror(retcode) << dendl;
	return set_cr_error(retcode);
      }
    }
