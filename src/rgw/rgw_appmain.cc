...
void rgw::AppMain::init_storage()
{
  ...
  env.store = StoreManager::get_storage(dpp, dpp->get_cct(),
          cfg,
          run_gc,
          run_lc,
          run_quota,
...
{
   rgw_rest_init(g_ceph_context, env.store->get_zone()->get_zonegroup());
  ...
  if (have_http_frontend) {
    ...
    if (apis_map.count("s3") > 0 || s3website_enabled) {
      if (!swift_at_root) {
        rest.register_default_mgr(set_logging(
            rest_filter(env.store, RGW_REST_S3,
    ...
    if (apis_map.count("swift") > 0) {
      ...
      if (! swift_at_root) {
        rest.register_resource(g_conf()->rgw_swift_url_prefix,
                            set_logging(rest_filter(env.store, RGW_REST_SWIFT,
                                                    swift_resource)));
      } else {
        if (env.store->get_zone()->get_zonegroup().get_zone_count() > 1) {
    ...
    if (apis_map.count("admin") > 0) {
      ...
      env.store->register_admin_apis(admin_resource);
...
{
  rgw_log_usage_init(dpp->get_cct(), env.store);
  ...
  olog_manifold->add_sink(new OpsLogRados(env.store));
  olog = olog_manifold;
} /* init_opslog */
...
int rgw::AppMain::init_frontends2(RGWLib* rgwlib)
{
  ...
  auto auth_registry =
    rgw::auth::StrategyRegistry::create(dpp->get_cct(), *(implicit_tenant_context.get()), env.store);
  ...
  r = env.store->register_to_service_map(dpp, daemon_type, service_map_meta);
  if (r < 0) {
    derr << "ERROR: failed to register to service map: " << cpp_strerror(-r) << dendl;
    /* ignore error */
  }

  if (env.store->get_name() == "rados") {
    // add a watcher to respond to realm configuration changes
    pusher = std::make_unique<RGWPeriodPusher>(dpp, env.store, null_yield);
