...
void RGWOp_BILog_List::execute(optional_yield y) {
  ...
  do {
    ...
    int ret = store->svc()->bilog_rados->log_list(s, bucket_info, log_layout, shard_id,
...
void RGWOp_BILog_Delete::execute(optional_yield y) {
  ...
  op_ret = store->svc()->bilog_rados->log_trim(s, bucket_info, log_layout, shard_id, start_marker, end_marker);
  if (op_ret < 0) {
