                                                       ...
                                                       bool use_gc, optional_yield y)
{
  ...
  if (is_driver) {
    ...
    if (dl) {
      ...
      if (newDriver)
        driver = newDriver(dpp, cct, io_context, site_config, false, use_gc_thread, use_lc_thread, quota_threads, run_sync_thread, run_reshard_thread, run_notification_thread, use_cache, use_gc, y);
      // if (dlclose(dl) < 0)
...
rgw::sal::Driver *DriverManager::init_raw_storage_provider(const DoutPrefixProvider* dpp, CephContext* cct,
                                                           const PluginConfig& cfg, boost::asio::io_context& io_context,
                                                           const rgw::SiteConfig& site_config)
{
  rgw::sal::Driver *driver = nullptr;
  rgw::sal::Driver *next = nullptr;
  ...
  if (is_driver) {
    ...
    if (dl) {
      newDriver = (rgw::sal::rgw_sal_new_driver_fn)dlsym(dl, "new_Driver");
      if (newDriver)
        driver = newDriver(dpp, cct, io_context, site_config, true, false, false, false, false, false, false, false, false, null_yield);
      // if (dlclose(dl) < 0)
      //  ldpp_dout(dpp, 0) << "WARNING: dlclose() failed" << dendl;
    }
    driver->dl = dl;
...
void DriverManager::close_storage(rgw::sal::Driver *driver)
{
  ...
  dl = driver->dl; driver->dl = nullptr;
...
DriverManager::PluginConfig DriverManager::get_config(bool admin, CephContext* cct)
{
