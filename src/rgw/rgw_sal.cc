						     ...
						     bool use_gc)
{
  ...
  ldpp_dout(dpp, 0) << "Filter name is: "<< cfg.filter_name << dendl;
  ...
  if (config_filter == "base") {
    ...
    if (d3n) {
      if (g_conf().get_val<Option::size_t>("rgw_max_chunk_size") !=
        ...
        lsubdout(cct, rgw, 0) << "rgw_d3n:  WARNING: D3N DataCache disabling (D3N requires that the chunk_size equals stripe_size)" << dendl;
      } else if (!g_conf().get_val<bool>("rgw_beast_enable_async")) {
	      lsubdout(cct, rgw, 0) << "rgw_d3n:  WARNING: D3N DataCache disabling (D3N requires yield context - rgw_beast_enable_async=true)" << dendl;
      } else {
        cfg.filter_name = "d3n";
      }
    }
