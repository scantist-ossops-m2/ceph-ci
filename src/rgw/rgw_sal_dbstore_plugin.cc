...
void *new_dbstore(const DoutPrefixProvider *dpp,
                 CephContext *cct,
                 bool raw,
                 bool use_gc_thread,
                 bool use_lc_thread,
...
void *new_Store(const DoutPrefixProvider *dpp,
                 ...
                 optional_yield opt_yield)
{
  rgw::sal::DBStore* driver = (rgw::sal::DBStore*)
         new_dbstore(dpp, cct, raw, use_gc_thread, use_lc_thread,
                     quota_threads, run_sync_thread, run_reshard_thread,
                     run_notification_thread, use_cache, use_gc, opt_yield);
  if (driver)
