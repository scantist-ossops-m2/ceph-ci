...
template<>
constexpr const char * signature_hash_name<ceph::crypto::HMACSHA1>;
template<>
constexpr const char * signature_hash_name<ceph::crypto::HMACSHA256>;
template<>
constexpr const char * signature_hash_name<ceph::crypto::HMACSHA512>;

template <class HASHVALUE>
const uint32_t signature_hash_name_size;
template<>
const uint32_t signature_hash_name_size<ceph::crypto::HMACSHA1>;
template<>
const uint32_t signature_hash_name_size<ceph::crypto::HMACSHA256>;
template<>
const uint32_t signature_hash_name_size<ceph::crypto::HMACSHA512>;

template <class HASHVALUE>
class SignatureHelperT {
protected:
  static constexpr uint32_t hash_size = signature_hash_size<HASHVALUE>;
...
template <typename HASHVALUE, SignatureFlavor SIGNATUREFLAVOR>
class FormatSignature {
};

// hexadecimal
template <typename HASHVALUE>
class FormatSignature<HASHVALUE, SignatureFlavor::BARE_HEX> : public SignatureHelperT<HASHVALUE> {
  using UCHARPTR = const unsigned char*;
  using base_t = SignatureHelperT<HASHVALUE>;
public:
  const char *result() {
    buf_to_hex((UCHARPTR) base_t::dest,
      signature_hash_size<HASHVALUE>,
      base_t::dest_str);
...
// prefix:base64
template <typename HASHVALUE>
class FormatSignature<HASHVALUE, SignatureFlavor::NAMED_BASE64> : public SignatureHelperT<HASHVALUE> {
  using UCHARPTR = const unsigned char*;
  using base_t = SignatureHelperT<HASHVALUE>;
public:
  char * const result() {
    const char *prefix = base_t::signature_name;
    const char *prefix_size = base_t::signature_name_size;
