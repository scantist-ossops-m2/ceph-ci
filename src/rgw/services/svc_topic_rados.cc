...
RGWTopicMetadataHandler::RGWTopicMetadataHandler(rgw::sal::Store* store,
...
RGWMetadataObject* RGWTopicMetadataHandler::get_meta_obj(
  ...
  return new RGWTopicMetadataObject(topic, objv, mtime, store);
}
...
int RGWTopicMetadataHandler::do_get(RGWSI_MetaBackend_Handler::Op* op,
  ...
  RGWTopicMetadataObject* rdo =
      new RGWTopicMetadataObject(result, ver, mtime, store);
  *obj = rdo;
  return 0;
}
...
class RGWMetadataHandlerPut_Topic : public RGWMetadataHandlerPut_SObj {
 ...
 public:
  ...
  int put_checked(const DoutPrefixProvider* dpp) override {
    ...
    const RGWPubSub ps(store, topic.user.tenant);
    return ps.create_topic(dpp, topic, y);
  }
};
