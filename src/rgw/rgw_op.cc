...
void RGWGetObj::execute(optional_yield y)
{
  ...
  if (attr_iter != attrs.end()) {
	...
	if (zone == dst_zone_trace) {
	  ...
	  ldpp_dout(this, 4) << "Object already has been copied to this destination. Returning " << op_ret << dendl;
	  goto done_err;
	}
      }
...
void RGWPutObj::execute(optional_yield y)
{
  ...
  if (multipart) {
    ...
    processor = store->get_append_writer(this, s->yield, s->object.get(),
    ...
    processor = store->get_atomic_writer(this, s->yield, s->object.get(),
					 s->bucket_owner.get_id(),
...
void RGWPostObj::execute(optional_yield y)
{
  ...
  do {
    ...
    std::unique_ptr<rgw::sal::Writer> processor;
    processor = store->get_atomic_writer(this, s->yield, obj.get(),
					 s->bucket_owner.get_id(),
