...
next:
  ...
  if (opt_cmd == OPT::OBJECT_REWRITE) {
    ...
    if (need_rewrite) {
      RGWRados* radosStore = static_cast<rgw::sal::RadosStore*>(store)->getRados();
      ret = radosStore->rewrite_obj(bucket->get_info(), obj->get_obj(), dpp(), null_yield);
      if (ret < 0) {
	...
	--expansion_factor;
	  ...
	  std::unique_ptr<rgw::sal::Object> obj = bucket->get_object(key);
          ...
          if (!need_rewrite) {
            ...
            RGWRados* radosStore = static_cast<rgw::sal::RadosStore*>(store)->getRados();
            r = radosStore->rewrite_obj(bucket->get_info(), obj->get_obj(), dpp(), null_yield);
            if (r == 0) {
...
#define MAX_LC_LIST_ENTRIES 100
 ...
 if (opt_cmd == OPT::RESHARD_STALE_INSTANCES_DELETE) {
  ...
  if (opt_cmd == OPT::PUBSUB_TOPIC_LIST) {
    ...
    if (store->get_zone()->get_zonegroup().supports_feature(
      ...
      for (const auto& [_, topic] : result.topics) {
        std::set<std::string> subscribed_buckets;
        ret = store->get_bucket_topic_mapping(topic, subscribed_buckets,
                                              null_yield, dpp());
        if (ret < 0) {
  ...
  if (opt_cmd == OPT::PUBSUB_NOTIFICATION_RM) {
    ...
    if (store->get_zone()->get_zonegroup().supports_feature(
            rgw::zone_features::notification_v2)) {
      ret = remove_notification_v2(dpp(), store, bucket.get(), notification_id,
                                   null_yield);
    } else {
      RGWPubSub ps(store, tenant);
