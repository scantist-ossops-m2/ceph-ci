...
//----------------------------------------------------------------------------
void SimpleBitmap::__set_single_bit_no_bounderies_check(uint64_t offset)
{
  uint64_t word_index = offset_to_index(offset);
  uint64_t bit_offset = (offset & BITS_IN_WORD_MASK);
  uint64_t set_mask   = 1ULL << bit_offset;
...
bool SimpleBitmap::set(uint64_t offset, uint64_t length)
{
  ...
  if (length == 1) {
    __set_single_bit_no_bounderies_check(offset);
    return true;
  }
  ...
  std::memset(&(m_arr[index]), 0xFF, words_to_bytes(full_words_count));
  index  += full_words_count;
...
//----------------------------------------------------------------------------
void SimpleBitmap::__clr_single_bit_no_bounderies_check(uint64_t offset)
{
  uint64_t word_index = offset_to_index(offset);
  uint64_t bit_offset = (offset & BITS_IN_WORD_MASK);
  uint64_t set_mask   = 1ULL << bit_offset;
...
bool SimpleBitmap::clr(uint64_t offset, uint64_t length)
{
  ...
  // special case optimization
  if (length == 1) {
    __clr_single_bit_no_bounderies_check(offset);
    return true;
  }
  ...
  uint64_t full_words_count = bits_to_words(length);
  std::memset(&(m_arr[index]), 0x0, words_to_bytes(full_words_count));
  index  += full_words_count;
  length -= words_to_bits(full_words_count);
...
extent_t SimpleBitmap::get_next_set_extent(uint64_t offset)
{
  ...
  // find the first clear (zero) bit (after reversing)
  // ffs is 1 based, must dec by one as we are zero based
