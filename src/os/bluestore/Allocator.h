...
protected:
  ...
  public:
    ...
    void foreach(std::function<void(T)> notify) {
   ...
   * Implemented via a set of independent buckets (aka LocklessWillfulStack).
  ...
  class WillfulExtentCache {
    ...
    std::vector<LocklessWillfulStack<uint64_t>> buckets;
  ...
  public:
    ...
    bool try_put(uint64_t offset, uint64_t len) {
      ...
      bool ret = false;
      ceph_assert(p2_isaligned(offset, myTraits.base_bits));
      ceph_assert(p2_isaligned(len, myTraits.base_bits));
