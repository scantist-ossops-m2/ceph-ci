...
bool BlueStore::BufferSpace::_dup_writing(TransContext* txc, BufferCacheShard* cache, OnodeRef onode)
{
  ...
  if (!writing.empty()) {
    ...
    for (auto it = writing.begin(); it != writing.end(); ++it) {
      Buffer& b = *it;
      Buffer* to_b = new Buffer(&onode->bc, b.state, b.seq, b.offset, b.data, b.flags);
...
int BlueStore::BufferSpace::_discard(BufferCacheShard* cache, uint32_t offset, uint32_t length)
{
  ...
  while (i != buffer_map.end()) {
    ...
    if (b->data.length()) {
      ...
      _add_buffer(
          cache, new Buffer(this, b->state, b->seq, end, keep, b->flags), 0, b);
    }
    _rm_buffer(cache, i);
                        ...
                        << dendl;
  {
    ...
    for (auto i = _data_lower_bound(offset);
      ...
      if (val) {
        ...
        if (b->length > length) {
          // take what's left that we can read and finish
          res[offset].substr_of(b->data, 0, length);
          res_intervals.insert(offset, length);
          break;
        } else {
          // take this buffer and continue, there might be more
