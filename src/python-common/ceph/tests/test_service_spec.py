...
@pytest.mark.parametrize(
    ...
    [
        ...
        ('*', "PlacementSpec(host_pattern=HostPattern(pattern='*', is_regex=False))"),
        ('3 data[1-3]', "PlacementSpec(count=3, host_pattern=HostPattern(pattern='data[1-3]', is_regex=False))"),
        ('3 data?', "PlacementSpec(count=3, host_pattern=HostPattern(pattern='data?', is_regex=False))"),
        ('3 data*', "PlacementSpec(count=3, host_pattern=HostPattern(pattern='data*', is_regex=False))"),
        ("count-per-host:4 label:foo", "PlacementSpec(count_per_host=4, label='foo')"),
        ('regex:Foo[0-9]|Bar[0-9]', "PlacementSpec(host_pattern=HostPattern(pattern='Foo[0-9]|Bar[0-9]', is_regex=True))"),
        ('3 regex:Foo[0-9]|Bar[0-9]', "PlacementSpec(count=3, host_pattern=HostPattern(pattern='Foo[0-9]|Bar[0-9]', is_regex=True))"),
    ])
