...
class NvmeofServiceSpec(ServiceSpec):
    ...
    #TODO(redo): what other parameters should be validated as part of this function?
    def validate(self) -> None:
        ...
        if self.enable_auth:
            if not any([self.server_key, self.server_cert, self.client_key, self.client_cert]):
                raise SpecValidationError('enable_auth is true but client/server certificates are missing')
