...
void DaemonMetricCollector::dump_asok_metrics() {
  ...
  for (auto &[daemon_name, sock_client] : clients) {
    ...
    std::cout << "welcome :"<< labeledperf << std::endl;
    if (labeledperf) {
      ...
      std::cout << "labeled dump: " << labeled_perf_dump_response << std::endl;
      ...
      std::cout << "yayy " << labeledperf << std::endl;
      ...
      std::cout << "chamayla " << labeled_perf_group_object << std::endl;
      for (auto &labeled_perf_counter : labeled_perf_group_object) {
        ...
        if (labeled_perf_group.find("z_rgw") == 0 || labeled_perf_group.find("rbd:") == 0) {
        std::vector<std::string> result;
        labels_t labels;
        boost::split(result, labeled_perf_group, boost::is_any_of(":"));
        std::cout << "result kya aya: " << result << std::endl;
        std::string labeled_name = "ceph_" + result[0] + "_" + labeled_perf_name;
        std::cout << "labeled metric name: " << labeled_name << std::endl;

        // fetch labels from name
        result.pop_back();
