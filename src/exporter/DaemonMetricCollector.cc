...
void DaemonMetricCollector::send_requests() {
  ...
  for(auto client : clients) {
    ...
    // std::cout << "st1: " << daemon_name << std::endl;
    ...
    if (response.size() > 0) {
      ...
      if (daemon_name.find("rgw") != std::string::npos || daemon_name.find("rbd-mirror") != std::string::npos) {
        std::cout << "rgw/rbd: " << schema << std::endl;
    }
      for (auto perf : schema) {
        ...
        for (auto perf_counter : perf_group_object) {
          ...
          if (daemon_name.find("rgw") != std::string::npos) {
            ...
            std::cout << "rbd: " << daemon_name << std::endl;
