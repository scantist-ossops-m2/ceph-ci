...
static void usage()
{
  std::cout << "Usage: rbd-nbd [options] map <image-or-snap-spec>    Map image to nbd device\n"
            ...
            << "                                (possible values: luks1, luks2, luks)\n"
...
static int do_map(int argc, const char *argv[], Config *cfg, bool reconnect)
{
  ...
  if (encryption_format_count > 0) {
    ...
    std::vector<librbd::encryption_luks1_format_options_t> luks1_opts;
    std::vector<librbd::encryption_luks2_format_options_t> luks2_opts;
    std::vector<librbd::encryption_luks_format_options_t> luks_opts;

    auto sg = make_scope_guard([&] {
      for (auto& opts : luks1_opts) {
        auto& passphrase = opts.passphrase;
        ceph_memzero_s(&passphrase[0], passphrase.size(), passphrase.size());
      }
      for (auto& opts : luks2_opts) {
        auto& passphrase = opts.passphrase;
        ceph_memzero_s(&passphrase[0], passphrase.size(), passphrase.size());
      }
    ...
    luks1_opts.reserve(encryption_format_count);
    luks2_opts.reserve(encryption_format_count);
    ...
    for (size_t i = 0; i < encryption_format_count; ++i) {
      ...
      switch (specs[i].format) {
        case RBD_ENCRYPTION_FORMAT_LUKS1: {
          luks1_opts.emplace_back();
          specs[i].opts = &luks1_opts.back();
          specs[i].opts_size = sizeof(luks1_opts.back());
          passphrase = &luks1_opts.back().passphrase;
          break;
        }
        case RBD_ENCRYPTION_FORMAT_LUKS2: {
          luks2_opts.emplace_back();
          specs[i].opts = &luks2_opts.back();
          specs[i].opts_size = sizeof(luks2_opts.back());
          passphrase = &luks2_opts.back().passphrase;
          break;
        }
    ...
    if (encryption_format_count == 1) {
      r = image.encryption_load(
                specs[0].format, specs[0].opts, specs[0].opts_size);
    } else {
  ...
  for (i = args.begin(); i != args.end(); ) {
    if (ceph_argparse_flag(args, i, "-h", "--help", (char*)NULL)) {
      ...
      *err_msg << "rbd-nbd: --timeout is deprecated (use --io-timeout)";
      ...
      if (arg_value == "luks1") {
        cfg->encryption_format.push_back(RBD_ENCRYPTION_FORMAT_LUKS1);
      } else if (arg_value == "luks2") {
        cfg->encryption_format.push_back(RBD_ENCRYPTION_FORMAT_LUKS2);
