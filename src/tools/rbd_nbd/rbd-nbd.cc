...
static int do_map(int argc, const char *argv[], Config *cfg, bool reconnect)
{
  ...
  if (encryption_format_count > 0) {
    ...
    // luks{1,2} block size can vary upto 4096, while luks1 always uses 512
    // currently we don't have an rbd API for figuring out the block size
    blksize = 4096;
  }
