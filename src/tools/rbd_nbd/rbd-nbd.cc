...
static int netlink_resize(int nbd_index, uint64_t size, std::string cookie);
...
private:
  ...
  void handle_notify_entry() {
    ...
    while (wait_notify()) {
      ...
      if (use_netlink) {
        ret = netlink_resize(nbd_index, new_size, cookie);
        if (ret < 0) {
          derr << "netlink resize failed: " << cpp_strerror(errno) << dendl;
        }
      } else {
