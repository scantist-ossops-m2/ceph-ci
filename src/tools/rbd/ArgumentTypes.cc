...
void add_encryption_options(boost::program_options::options_description *opt) {
  ...
  (ENCRYPTION_FORMAT.c_str(),
   ...
   "encryption formats [possible values: luks1, luks2, luks]");

  opt->add_options()
    (ENCRYPTION_PASSPHRASE_FILE.c_str(),
     po::value<std::vector<std::string>>(),
     "path of files containing passphrase for unlocking the image");
}
...
void validate(boost::any& v, const std::vector<std::string>& values,
  ...
  if (s == "luks") {
    ...
    format.format = RBD_ENCRYPTION_FORMAT_LUKS1;
  } else if (s == "luks2") {
    format.format = RBD_ENCRYPTION_FORMAT_LUKS2;
  } else {
    throw po::validation_error(po::validation_error::invalid_option_value);
  }
