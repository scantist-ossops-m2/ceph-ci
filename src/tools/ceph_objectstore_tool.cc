    ...
    for (vector<ghobject_t>::iterator obj = list.begin();
	 ...
	 ++obj) {
      ...
      if (coll != coll_t::meta()) {
        ...
        if (r < 0 ) {
	  // error is expected on head object
	  if (!obj->hobj.is_head()) {
	    cerr << "(1)Error getting attr on : " << make_pair(coll, *obj) << ", "
		 << cpp_strerror(r) << std::endl;
	  }
        } else {
	  auto bp = attr.cbegin();
...
int ObjectStoreTool::do_import(ObjectStore *store, OSDSuperblock& sb,
			       bool force, std::string pgidstr, GlobalSnapMapper *gsnap_mapper)
{
  bufferlist ebl;
  pg_info_t info;
  PGLog::IndexedLog log;
			 ...
			 enum rmtype type)
{
  if ( (type == BOTH || type == SNAPMAP) && !ghobj.hobj.is_head() ) {
    cout << __func__ << "::GBH::remove_oid()::hobj=" << ghobj.hobj << ", snaps=" << snaps << std::endl;
  ...
  if (type == BOTH || type == NOSNAPMAP) {
    ...
    cout << __func__ << "::GBH::remove_oid()::SNAPMAP only!" << std::endl;
  }
  return 0;
}
...
int get_snapset_for_coid(ObjectStore *store, coll_t coll, ghobject_t &ghobj, std::vector<snapid_t> & snaps)
{
  SnapSet ss;
  // SS_ATTR are stored on the head object
  ghobject_t head(ghobj);
  head.hobj = head.hobj.get_head();
  int r = get_snapset(store, coll, head, ss, false);
  if (r < 0) {
    cerr << "Can't get snapset error " << cpp_strerror(r) << std::endl;
    return r;
  }
  else if (ss.clone_snaps.empty()) {
    cerr << "Empty snapset" << std::endl;
    return -1;
  }

  auto itr = ss.clone_snaps.find(ghobj.hobj.snap);
  if (itr != ss.clone_snaps.end()) {
    snaps = itr->second;
    return 0;
  }
  else {
    cerr << "No Valid SNAPSET for snapid=" << ghobj.hobj.snap << std::endl;
    return -1;
  }
}
		 ...
		 bool force, enum rmtype type)
{
  ...
  // This code block seems to be doing nothing as snaps are always empty
  ...
  if (!dry_run) {
    ...
    if (ghobj.hobj.is_head()) {
      cout << "GBH:: remove head object " << ghobj << std::endl;
    }
    else if (type == NOSNAPMAP) {
      cout << "GBH:: remove object without snapmap " << ghobj << std::endl;
    }
    else {
      if (get_snapset_for_coid(store, coll, ghobj, snaps) == 0) {
	cout << "GBH:: remove clone object " << ghobj << ", snaps=" << snaps << std::endl;
      }
      else {
	return -1;
      }
...
GlobalSnapMapper* create_and_restore_snap_mapper(ObjectStore *store, CephContext *cct, const char *caller)
{
  GlobalSnapMapper *gsnap_mapper = new GlobalSnapMapper(cct->get());
  ceph_assert(gsnap_mapper != nullptr);
  store->restore_snap_mapper(*gsnap_mapper, caller);
  return gsnap_mapper;
}
...
int main(int argc, char **argv)
{
  ...
  desc.add_options()
    ...
    ("op", po::value<string>(&op),
     "Arg is one of [info, log, remove, mkfs, fsck, repair, fuse, dup, export, export-remove, import, list, listsnap, list-slow-omap, fix-lost, list-pgs, dump-super, meta-list, "
  ...
  if ((op == "list" || op == "listsnap" || op == "meta-list") && human_readable) {
  ...
  if (op == "listsnap") {
#if 1
    // This is a debug tool able to display the first 64 objs in a snap
    // TBD:  add count paramter (using default 16)
    // TBD2: add shard paramter (using shard_id_t::NO_SHARD)
    const std::string & snapid_str = object;
    snapid_t snapid;
    try {
      snapid = std::stoi(snapid_str);
    }
    catch(...) {
      std::cerr << "bad snapid " << snapid_str << std::endl;
      ret = -EINVAL;
      goto out;
    }
    std::vector<hobject_t> out;
    unsigned max_count = 16;
    SnapMapperShard sm_shard(shard_id_t::NO_SHARD);
    const auto gsnap_mapper = create_and_restore_snap_mapper(fs.get(), cct.get(), "listsnap");
    int ret = gsnap_mapper->get_objs_for_scrubber(sm_shard, snapid, out, max_count);
    if (ret == 0) {
      std::cout << "snapid = " << snapid <<  std::endl;
      for (const hobject_t& coid : out) {
	std::cout << coid << std::endl;
      }
    }
    else {
      cerr << "snapid <" << snapid << "> doesn't exist!" << std::endl;
      ret = -EINVAL;
    }
    goto out;
#endif
  ...
  if (op == "import") {
    ...
    auto gsnap_mapper = create_and_restore_snap_mapper(fs.get(), cct.get(), "do_import");
    try {
      ret = tool.do_import(fs.get(), *superblock, force, pgidstr, gsnap_mapper);
    }
    catch (const buffer::error &e) {
      cerr << "do_import threw exception error " << e.what() << std::endl;
      ret = -EFAULT;
    }
    gsnap_mapper = store_and_dispose_snap_mapper(gsnap_mapper, fs.get(), "do_import");
    if (ret == -EFAULT) {
  ...
  if (op == "remove") {
    ...
    auto gsnap_mapper = create_and_restore_snap_mapper(fs.get(), cct.get(), "remove");
    ret = initiate_new_remove_pg(gsnap_mapper, fs.get(), pgid);
    store_and_dispose_snap_mapper(gsnap_mapper, fs.get(), "remove");
    if (ret < 0) {
...
// The following code for export, info, log require omap or !skip-mount-omap
  if (it != ls.end()) {
	...
	auto gsnap_mapper = create_and_restore_snap_mapper(fs.get(), cct.get(), "objcmd");
        ret = do_remove_object(pg_num_history, gsnap_mapper, fs.get(), coll, ghobj, all, force, type);
	store_and_dispose_snap_mapper(gsnap_mapper, fs.get(), "objcmd");
        goto out;
    ...
    if (op == "export" || op == "export-remove") {
      ...
      if (ret == 0) {
        ...
        if (op == "export-remove") {
	  auto gsnap_mapper = create_and_restore_snap_mapper(fs.get(), cct.get(), "export-remove");
          ret = initiate_new_remove_pg(gsnap_mapper, fs.get(), pgid);
	  store_and_dispose_snap_mapper(gsnap_mapper, fs.get(), "export-remove");
