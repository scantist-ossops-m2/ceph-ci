        ...
        if self.lvs:
            ...
            if self.path[0] == '/':
                if self.path.startswith('/dev/mapper'):
                    lv_path = lvm.get_lv_path_from_mapper(os.path.basename(self.path))
                else:
                    lv_path = self.path
...
class ValidZapDevice(ValidDevice):
    ...
    def check_device(self):
        ...
        if not self.force:
            ...
            if self.device.lvs:
                osd_ids = [lv.tags['ceph.osd_id'] for lv in self.device.lvs]

            self.check_running_osd(osd_ids)

        return self.format_device()

    def check_osd_id(self, osd_id):
        # TODO: disk.is_locked_raw_device() should be called
        #
        self.check_running_osd([osd_id])
