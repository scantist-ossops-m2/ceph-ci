...
class Device(object):
    ...
    def __init__(self, path, with_lsm=False, lvs=None, lsblk=None, all_devices_vgs=None):
        self.path = path
        ...
        self.lsblk = lsblk if lsblk else disk.lsblk(self.path)
    ...
    def _parse(self):
        ...
        if lv:
            ...
            if self.lsblk:
                for dev in self.lsblk:
                    if dev['NAME'] == os.path.basename(self.path):
                        break
            else:
                dev = disk.lsblk(device=self.path)
