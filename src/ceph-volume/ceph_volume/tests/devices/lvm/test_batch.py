...
#        conf_ceph_stub('[global]\nfsid=asdf-lkjh')
#        data_device = mock_device_generator(name='data')
#        args = factory(block_db_slots=None,
#                       get_block_db_size=None,
#                       devices=data_device.path)
#        dev_size = 21474836480
...
#        fasts = batch.get_physical_fast_allocs(mock_devices_available_multi_pvs_per_vg,
#                                              'block_db', slots_per_device, 2, args)
...
#            assert slot_size == expected_slot_size
    ...
    def test_get_physical_fast_allocs_length_existing(self,
        ...
        devs = [mock_device_generator(name='foo', suffix=str(_)) for _ in range(slots)]
        dev_paths = [dev.path for dev in devs]
        fast_devs = [mock_device_generator(name='ssd', suffix=str(_)) for _ in range(num_devs)]
        already_assigned = 0
