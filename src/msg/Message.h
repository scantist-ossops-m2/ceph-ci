...
class Message : public RefCountedObject {
 ...
 protected:
  ...
  ceph::buffer::list
    data;  // data payload (page-alignment will be preserved where possible)
 ...
 public:
  ...
  typedef boost::intrusive::list<
    Message,
    boost::intrusive::member_hook<Message,
 ...
 public:
  ...
  Message* get() { return static_cast<Message*>(RefCountedObject::get()); }
 ...
 public:
  ...
  void set_connection(ConnectionRef c) { connection = std::move(c); }
  ...
  void set_byte_throttler(ThrottleInterface* t) { byte_throttler = t; }
  void set_message_throttler(ThrottleInterface* t) { msg_throttler = t; }
  ...
  void calc_data_crc() { footer.data_crc = data.crc32c(0); }

  virtual int get_cost() const { return data.length(); }
  ...
  entity_name_t get_source() const { return entity_name_t(header.src); }
  ...
  // forwarded?
  entity_inst_t get_orig_source_inst() const { return get_source_inst(); }
  entity_name_t get_orig_source() const { return get_source(); }
  entity_addr_t get_orig_source_addr() const { return get_source_addr(); }
  entity_addrvec_t get_orig_source_addrs() const { return get_source_addrs(); }
...
extern void encode_message(Message* m,
                           uint64_t features,
                           ceph::buffer::list& bl);
extern Message* decode_message(CephContext* cct,
                               int crcflags,
...
/// reference leaks by forcing . Instead, you must either cast the derived class
/// to a RefCountedObject to do the get/put or detach a temporary reference.
