...
#. After all the existing custom mClock configuration settings have been removed
   ...
   ``high_client_ops`` will come into effect. This can be verified by using
...
.. note:: The configuration changes on an OSD using the below commands are
   ...
   further using the *ceph config set osd.N ...* command. The changes wouldn't
   take effect until the OSD is restarted and this is as per the config
   subsystem design. However, any further modification can still be made
