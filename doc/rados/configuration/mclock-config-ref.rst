...
For example, to change the profile to *custom* on all the OSDs, the following
command can be used:
...
.. tip::  The reservation and limit parameter allocations must be on a per shard
   based on the type of backing device (HDD/SSD) under the OSD. See
...
additional intermediate step of removing the custom settings from the
central config database for the changes to take effect.
...
#. Set the desired built-in profile using:
   ...
   For example, to set the built-in profile to ``high_client_ops`` on all the
...
#. Remove the custom mClock configuration settings determined in the previous
   ...
   :confval:`osd_mclock_scheduler_client_res` that was set on all the OSDs, run
   the following command:

   .. prompt:: bash #

     ceph config rm osd osd_mclock_scheduler_client_res
#. After all the existing custom mClock configuration settings have been removed
   from the central config database, the configuration settings pertaining to
   ``high_client_ops`` should come into effect. This can be verified by using
   the *"ceph config show osd.N"* command.
...
.. note:: The configuration changes using the below commands are ephemeral and
   are lost in case the OSD restarts. It is also important to note that the
   config options overridden using the below commands cannot be modified further
   using the *ceph config set osd.N ...* command. The changes wouldn't take
   effect until the OSD is restarted and this is as per the config subsystem
   design. However, any further modification can still be ephemerally
   made using the commands mentioned below.

#. Run the *injectargs* command as shown in the following example to override
   the mclock settings:
...
ephemerally modified using the above commands.
