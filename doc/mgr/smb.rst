...
custom_dns
    ...
    not configured to resolve an DNS within AD domain(s).
placement
...
Each resource description must belong to one of the types described below.
...
restriction only one Samba server, as part of one cluster, may run a single
