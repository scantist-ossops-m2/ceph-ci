...
.. prompt:: bash #
    ...
    ceph orch host ls [--format yaml] [--matches-name <name>] [--label <label>] [--host-status <status>]

where the optional arguments "matches-name", "label" and "host-status" are used for filtering.
"matches-name" is a regex that will match against hostnames and will only return matching hosts
