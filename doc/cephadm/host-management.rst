...
When this flag is specified, cephadm will make a best-effort attempt to remove the bucket
entry for the host from the CRUSH map. Note that if it fails to do so, the host will still
have been removed from cephadm's management, but its entry in the CRUSH map will remain.
