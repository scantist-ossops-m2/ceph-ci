...
Bootstrap allows users to create their own private/public SSHkey pair
rather than having cephadm generate them itself

To use custom SSH keys, pass the ``--ssh-private-key`` and ``--ssh-public-key``
fields to bootstrap. The parameters expect a filepath to the file where the
keys are stored. For example:
...
deployment using CA signed keys. Before bootstrapping it's recommended to setup
the CA public key as a trusted CA key on hosts you'd like to add to eventually
add to the cluster. For example:

.. prompt:: bash

  # going to be our own CA here, meaning we'll need to make a CA key
...
current example, we will make our own key and sign it with our CA key
