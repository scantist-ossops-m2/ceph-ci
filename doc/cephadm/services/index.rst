...
By default, the host pattern is matched using fnmatch:
...
The host pattern also has support for using a regex rather than fnmatch.
To use a regex, you must either add "regex: " to the start of the pattern
if using the command line, or specify a ``pattern_type`` field to be "regex"
if using YAML.
