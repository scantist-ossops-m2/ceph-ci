...
It may be needed to pause (write) IO to a set of subvolumes of a given file system.
...
and then issues the corresponding quiesce db command. You can learn more about the feature in the developer guides.
...
* **await** the set to change states, applicable as a modifier to other requests

All of the operations listed above are carried out syncronously by the designated quiesce leader,
which in the current design is always the daemon running rank 0 of the file system, unless an `await` modifier
is specified, in which case the specified action will be executed and then the request will be put
onto the await queue of the given set, and will be interrupted/completed by a state transition of the set.
...
again be `QUIESCED` and the expiration timer will restart. The expiration timer doesn't apply
when a set is `QUIESCING`

Awaiting
^^^^^^^^

Note that the commands above are all non-blocking. If we want to wait for the quiesce set
to reach the `QUIESCED` state, we should await it at some point. ``--await`` can be givent
...
When awaiting, one may also specify a maximum duration that they'd like this await request to block for,
...
.. note:: This example uses arbitrary timeouts to convey the concept. In real life, the values must be carefully
...
Sometimes, it's not enough to just observe the successful quesce or release. The reason could be
...
.. note:: When ``--if-version`` is used and the test fails, the requested action is NOT executed.
  It means that the script may want to execute a cancel on the set to prevent the unintended pause
  until the end of the expiration timer
